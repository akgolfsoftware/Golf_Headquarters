...
**Sannhetskilde-hierarki (ved konflikt vinner hoeyeste):**
...
| **Landing** | Moerk hero (`#0A0A0A`) | Lime CTA | Editorial |
...
| Token | Verdi | Bruk |
|---|---|---|
| `--color-primary` | `#005840` | Skogsgroenn, primaer brand. CTA-bg, sidebar-stripe (PlayerHQ active) |
| `--color-primary-hover` | `#00472f` | Hover paa primary |
...
| `--color-accent-hover` | `#C2EE2F` | Hover paa accent |
| `--color-accent-on` | `#0A1F18` | Tekst PAA lime-bg (moerk groenn, IKKE svart) |
...
### Tekst (Ink -- varmere graa-toner)

| Token | Verdi | Bruk |
|---|---|---|
| `--color-ink` | `#0A1F18` | Primaertekst (groenn-svart) |
| `--color-ink-muted` | `#5E5C57` | Sekundaertekst |
| `--color-ink-subtle` | `#9C9990` | Tertiaertekst (labels, captions) |
...
| Token | Verdi | Bruk |
|---|---|---|
| `--player-sidebar-bg` | `#FFFFFF` | PlayerHQ sidebar (LYS, ikke moerk) |
| `--player-sidebar-border` | `#F0EDE5` | PlayerHQ sidebar hoeyre-border |
| `--player-sidebar-active` | `rgba(0,88,64,0.08)` | PlayerHQ active item-bg |
| `--coach-rail-bg` | `#061210` | CoachHQ smal venstre-rail (moerk) |
| `--coach-nav-bg` | `#FAFAF7` | CoachHQ nav-kolonne (lys) |
| `--coach-nav-active` | `rgba(209,248,67,0.30)` | CoachHQ active item-bg (lime tint) |

> **VIKTIG KORREKSJON:** PlayerHQ sidebar er LYS (#FFFFFF), ikke moerk (#0A1F18) som tidligere antatt. CoachHQ har TO-LAGS struktur: smal moerk rail (56px) + lys nav-kolonne (200px).

### Landing

| Token | Verdi | Bruk |
|---|---|---|
| `--landing-hero-bg` | `#0A0A0A` | Moerk landing hero |
| `--landing-hero-text` | `#FFFFFF` | Hvit tekst paa hero |
...
| Bruk | Font | CSS-variabel | Stoerrelse | Tracking |
|---|---|---|---|---|
...
Alle spacing skal vaere multipler av 8px.

**Tillatt (Tailwind v4):** `p-2 (8), p-4 (16), p-6 (24), p-8 (32), p-10 (40), p-12 (48), p-16 (64)`

**Unngaa:** `p-1 (4), p-3 (12), p-5 (20), p-7 (28), p-9 (36)` (med mindre tett ikon-gruppe -- 4px og 12px tillates da)

Generoes spacing mellom seksjoner (48-64px). Stram spacing inni kort (16-20px).
...
| Fil | Bruk |
|---|---|
| `ak-golf-logo-primary-on-light.svg` | Default paa lys bg (#FAFAF7, #FFFFFF) -- groenn + lime |
| `ak-golf-logo-primary-on-dark.svg` | Moerk bg (#0A0A0A), landing hero, dark mode |
| `ak-golf-logo-white-on-green.svg` | Paa ren primary groenn (#005840) |
| `ak-golf-logo-white-on-dark.svg` | Compact moerk UI, favicons, smaa stoerrelser |
| `ak-golf-logo-primary-mono.svg` | Print, ett-fargers eksport |
| `ak-golf-logo-black-mono.svg` | Print sort, fax, gravering |
| `ak-golf-logo-white-mono.svg` | Moerk bg uten lime-aksent |

**Regler:** Aldri paa lime. Aldri mindre enn 24px hoeyde i UI / 12mm i print. Visuell katalog i `branding-style-guide.html` seksjon 0.

## 11. Fotografi

- Golfbane i **desaturert varm tone** (ikke fargemettet)
- Glassmorfisk overlay over foto, **aldri frittstaaende**
- Landing hero: foto + moerk overlay 0.65 opacity

---

## 12. UX-konvensjoner (norsk-spesifikt)

- **Spraak:** Norsk bokmaal med ae, oe, aa overalt
- **Desimaltegn:** komma -- `78,5`
- **Dato:** "7. mai 2026" (lang) eller `07.05.26` (kort)
- **Tidsformat:** 24-timer -- `14:00`
- **Broeker over prosent:** "12/16 oekter" heller enn "75 %"
- **Ingen emojier** -- bruk lucide-ikoner

---

## 13. Anti-AI-regler (skal foelges av Claude Design)

1. Flat avatar-farger (ingen gradienter)
2. Asymmetriske grid-layouts (ikke uniform 3x1 / 4x1)
3. Ingen `translateY(-3px)` hover paa alt
4. Maks 1-2 eyebrows per skjerm
5. Variert seksjonslayout paa landing
6. Redusert pill-tetthet (ikke pille-spam)
7. Ingen ambient-glow dekorasjoner
8. Sterkere typografisk kontrast mellom nivaaer
...
- `SessionCard` (oekt med pyramide-stripe)
...
Naar en design-pakke trenger en av disse manglende komponentene, peker pakken til komponent-spec i `desktop-import/`.
...
|---|---|---|
| **Gratis** | Noeytral graa (`#9C9990`) | Default. Kun grunnleggende features |
| **Pro** | Primary groenn (`#005840`) | Mest vanlige betalende |
| **Elite** | Accent lime (`#D1F843`) | Topp-tier |

**Tier-gating-moenster:** Locked features vises som dempet (40% opacity) med lucide `Lock` og CTA "Oppgrader til {Pro/Elite}".
...
Hvis Claude Design forsoeker aa bruke noen av disse, AVVIS og bruk erstatningen.
...
Per session:
1. **Last opp `branding-style-guide.html`** som primaer systemreferanse
2. **Last opp denne fila** som tekstlig backup
3. Bekreft at Claude har lest begge foer du sender foerste skjerm-pakke
4. Per skjerm-pakke: foelg `wireframe/design-prompt-template.md`
