# AK Golf Platform — Modal — NewPlanModal (arketype-D-referanse)

## Identitet

- **Produkt:** CoachHQ
- **Trigger:** `+ Ny plan`-CTA på `/admin/plans` (CoachHQ Plans-list, batch 2 pakke 1)
...
- **Note:** Duplikat fra batch 2 pakke 19. Inkludert her som **arketype-D-referanse for modal-wizards** — bruk samme spec som batch 2-D leverte. Hvis allerede APPROVED i batch 2, hopp over.

## Designsystem

Bruk **`branding-style-guide.html`** + **`design-system-v2.md`** som lastet system-kontekst. CoachHQ-modal — bruker `card`-bg på lyst tema, `popover`-bg på mørkt. Maks 3 lime per modal.
...
- **Steg-indikator:** 4 dots under header (accent for aktiv, primary for fullført, muted for ufullført) + linjer mellom
- **Body (variabel høyde, max 70vh, scrollbar):**
  ...
  - **Steg 2 — Mal-valg:**
    - 3 store cards i grid: «Bruk mal →» (åpner TemplateSelectorModal — pakke 15) · «AI-generer →» (lime accent, åpner AIPlanGeneratorModal pakke 14) · «Start blank →»
...
- Ikke designe selve plan-detalj-skjermen som åpnes etter send
