...
Versjon: 3 (kontrollert 2026-09-12)
---
...
2. **Sikkerhet og personvern:** Hvis diffen rører `src/`, `prisma/`, `proxy.ts`,
   `vercel.json`, auth, betaling, filer, logger, AI eller `docs/gdpr/`: les
   [ak-sikkerhet](../ak-sikkerhet/SKILL.md) og
   [ak-personvern](../ak-personvern/SKILL.md). Ikke commit før de tre spørsmålene
   der er besvart med evidens fra koden, ikke antakelse. Last detaljskillen som
   treffer (Next, Stripe, RLS, opplasting, AI).
3. **Kjør gaten:** `npm run verify`
...
4. **Tolk feil med gotcha-lista** (`.claude/rules/gotchas.md`) FØR du fikser:
...
5. **Fiks og kjør gaten på nytt.** Gjenta til grønt. Aldri commit med rød gate,
   aldri `--no-verify`.
6. **Commit:** Conventional Commits på engelsk (`feat:`, `fix:`, `chore:`, `docs:` …),
