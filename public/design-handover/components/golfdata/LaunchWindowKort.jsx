 ...
 */
const CSS = `
.ak-lwk{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);
  padding:20px;display:flex;flex-direction:column;gap:12px;}
.ak-lwk__hode{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;}
.ak-lwk__eyebrow{font-family:var(--font-mono);font-size:var(--text-11);font-weight:600;
  letter-spacing:var(--tracking-eyebrow);text-transform:uppercase;color:var(--text-muted);}
.ak-lwk__tittel{font-family:var(--font-display);font-weight:700;font-size:var(--text-18);
  line-height:1.2;letter-spacing:var(--tracking-display);color:var(--text);margin:6px 0 0;}
.ak-lwk__sub{font-family:var(--font-mono);font-size:var(--text-11);color:var(--text-muted);margin-top:4px;}
.ak-lwk__hero{text-align:right;}
.ak-lwk__meter{font-family:var(--font-mono);font-size:var(--text-24);font-weight:700;
  font-variant-numeric:tabular-nums;color:var(--text);line-height:1.05;}
.ak-lwk__meterlbl{font-family:var(--font-mono);font-size:var(--text-11);color:var(--text-muted);margin-top:3px;}
.ak-lwk__svg{width:100%;height:auto;display:block;}
.ak-lwk__stats{font-family:var(--font-mono);font-size:var(--text-11);color:var(--text-muted);}
.ak-lwk__dom{font-family:var(--font-ui);font-size:var(--text-13);line-height:1.5;color:var(--text-2);margin:0;}
.ak-lwk__tomtekst{font-family:var(--font-ui);font-size:var(--text-13);line-height:1.55;color:var(--text-2);margin:0;}
.ak-lwk__skel{height:170px;border-radius:10px;background:var(--surface-2);}
`;
if (typeof document !== "undefined" && !document.getElementById("ak-lwk-css")) {
  const s = document.createElement("style"); s.id = "ak-lwk-css"; s.textContent = CSS; document.head.appendChild(s);
}
...
export function LaunchWindowKort({
  ...
  const tickTxt = { fontFamily: "var(--font-mono)", fontSize: 8.5, fill: "var(--text-faint)" };
