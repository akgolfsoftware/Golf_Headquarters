 ...
 */
const CSS = `
.ak-sgtr{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);padding:18px;}
.ak-sgtr__svg{width:100%;display:block;overflow:visible;}
.ak-sgtr__ev{font-family:var(--font-mono);font-size:8.5px;fill:var(--text-muted);}
`;
if (typeof document !== "undefined" && !document.getElementById("ak-sgtr-css")) {
  const s = document.createElement("style"); s.id = "ak-sgtr-css"; s.textContent = CSS; document.head.appendChild(s);
}
...
export function SgTrend({
  ...
  const x = (i) => padL + (i / (punkter.length - 1)) * (W - padL - padR);
