...
const out = "/private/tmp/ak-hq-workbench-natt-20260920/rigg";
...
const fonts = readdirSync(chunks).filter(f=>f.endsWith(".css")).flatMap(f=>[...readFileSync(resolve(chunks,f),"utf8").matchAll(/@font-face\{[^}]*font-family:(?:Geist|Archivo|Oswald|IBM)[^}]+\}/g)].map(([face])=>face.replace(/url\(([^)]+)\)/g,(_,url)=>`url(data:font/woff2;base64,${readFileSync(resolve(chunks,url.replace(/["']/g,""))).toString("base64")})`)));
if (!fonts.length) throw new Error("Appens fontfiler mangler");
...
createServer((req,res)=>{
  const path=new URL(req.url,"http://127.0.0.1:5460").pathname;
...
}).listen(5460,"127.0.0.1",()=>console.log("Workbench-komponentprøve: http://127.0.0.1:5460"));
