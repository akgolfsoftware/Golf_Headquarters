...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-turneringer.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-turneringer-${viewport}-${tema}.png`;
  test(`portal turneringer ${viewport} ${tema}`, async ({ page }) => {
