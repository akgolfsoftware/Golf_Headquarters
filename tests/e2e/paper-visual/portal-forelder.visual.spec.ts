...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/foreldreportal.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-forelder-${viewport}-${tema}.png`;
  test(`portal forelder ${viewport} ${tema}`, async ({ page }) => {
