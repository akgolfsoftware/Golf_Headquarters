...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-chat-mobil.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-coach-sporsmal-traad-${viewport}-${tema}.png`;
  test(`portal coach sporsmal traad ${viewport} ${tema}`, async ({ page }) => {
