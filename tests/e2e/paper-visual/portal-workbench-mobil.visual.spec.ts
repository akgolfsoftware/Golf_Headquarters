...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/workbench-mobil.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-workbench-mobil-${viewport}-${tema}.png`;
  test(`portal workbench-mobil ${viewport} ${tema}`, async ({ page }) => {
