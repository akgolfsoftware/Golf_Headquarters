...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-innboks.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-innboks-${viewport}-${tema}.png`;
  test(`admin innboks ${viewport} ${tema}`, async ({ page }) => {
