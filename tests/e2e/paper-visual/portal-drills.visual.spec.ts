...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-drills.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-drills-${viewport}-${tema}.png`;
  test(`portal drills ${viewport} ${tema}`, async ({ page }) => {
