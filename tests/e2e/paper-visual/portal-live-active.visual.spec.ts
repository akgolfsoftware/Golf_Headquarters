...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-live-okt.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-live-active-${viewport}-${tema}.png`;
  test(`portal live-active ${viewport} ${tema}`, async ({ page }) => {
