...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-teknisk-plan.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-teknisk-plan-${viewport}-${tema}.png`;
  test(`portal teknisk-plan ${viewport} ${tema}`, async ({ page }) => {
