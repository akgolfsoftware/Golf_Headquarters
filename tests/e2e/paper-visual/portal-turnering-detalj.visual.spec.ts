...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-turnering-detalj.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-turnering-detalj-${viewport}-${tema}.png`;
  test(`portal turnering-detalj ${viewport} ${tema}`, async ({ page }) => {
