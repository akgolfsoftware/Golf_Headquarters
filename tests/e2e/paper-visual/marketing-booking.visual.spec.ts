...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/booking.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `marketing-booking-${viewport}-${tema}.png`;
  test(`marketing booking ${viewport} ${tema}`, async ({ page }) => {
