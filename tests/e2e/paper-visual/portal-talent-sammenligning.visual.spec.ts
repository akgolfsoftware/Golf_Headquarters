...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-analyse.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-talent-sammenligning-${viewport}-${tema}.png`;
  test(`portal talent-sammenligning ${viewport} ${tema}`, async ({ page }) => {
