...
const FASIT = path.resolve(
  __dirname,
  "../../../designsystem/paper/fase2/playerhq/playerhq-live-tapper.html",
);
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-live-tapper-${viewport}-${tema}.png`;
  test(`live tapper ${viewport} ${tema}`, async ({ page }) => {
    await page.setViewportSize(PAPER_VIEWPORTS[viewport]);
    test.skip(!ER_SEED, "bygget rute krever sessionId + auth — seed fasit-baseline her");
