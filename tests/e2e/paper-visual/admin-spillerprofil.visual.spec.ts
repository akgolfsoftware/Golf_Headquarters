...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/spillerprofil.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-spillerprofil-${viewport}-${tema}.png`;
  test(`admin spillerprofil ${viewport} ${tema}`, async ({ page }) => {
