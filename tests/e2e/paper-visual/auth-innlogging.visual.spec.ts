...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/innlogging.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `auth-innlogging-${viewport}-${tema}.png`;
  test(`auth innlogging ${viewport} ${tema}`, async ({ page }) => {
