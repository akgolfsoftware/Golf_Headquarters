...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/workbench-turnering.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-workbench-turnering-${viewport}-${tema}.png`;
  test(`portal workbench-turnering ${viewport} ${tema}`, async ({ page }) => {
