...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-spillere.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-spillere-${viewport}-${tema}.png`;
  test(`admin spillere ${viewport} ${tema}`, async ({ page }) => {
