...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-test-gjennomfor.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-test-gjennomfor-${viewport}-${tema}.png`;
  test(`portal test-gjennomfor ${viewport} ${tema}`, async ({ page }) => {
    await page.setViewportSize(PAPER_VIEWPORTS[viewport]);
    test.skip(!ER_SEED, "seed-only: bygget rute krever auth");
