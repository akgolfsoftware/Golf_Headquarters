...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-runde-logg.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-runde-logg-${viewport}-${tema}.png`;
  test(`portal runde-logg ${viewport} ${tema}`, async ({ page }) => {
