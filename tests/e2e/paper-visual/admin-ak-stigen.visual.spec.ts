...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-ak-stigen.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-ak-stigen-${viewport}-${tema}.png`;
  test(`admin ak-stigen ${viewport} ${tema}`, async ({ page }) => {
