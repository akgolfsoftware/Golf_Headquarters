...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-okt-detalj.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-okt-detalj-${viewport}-${tema}.png`;
  test(`portal okt-detalj ${viewport} ${tema}`, async ({ page }) => {
