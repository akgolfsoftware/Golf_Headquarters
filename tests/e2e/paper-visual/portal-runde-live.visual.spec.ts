...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-runde-live.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-runde-live-${viewport}-${tema}.png`;
  test(`portal runde-live ${viewport} ${tema}`, async ({ page }) => {
