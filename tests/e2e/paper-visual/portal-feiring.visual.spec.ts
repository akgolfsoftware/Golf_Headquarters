...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-feiring.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-feiring-${viewport}-${tema}.png`;
  test(`portal feiring ${viewport} ${tema}`, async ({ page }) => {
