...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-live-summary.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-live-summary-${viewport}-${tema}.png`;
  test(`portal live-summary ${viewport} ${tema}`, async ({ page }) => {
    await page.setViewportSize(PAPER_VIEWPORTS[viewport]);
    test.skip(!ER_SEED, "seed-only: bygget rute krever auth + session");
