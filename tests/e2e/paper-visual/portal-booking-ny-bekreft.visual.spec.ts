...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-booking.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-booking-ny-bekreft-${viewport}-${tema}.png`;
  test(`portal booking ny bekreft ${viewport} ${tema}`, async ({ page }) => {
