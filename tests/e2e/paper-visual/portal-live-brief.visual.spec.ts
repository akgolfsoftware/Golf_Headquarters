...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/playerhq-live-brief.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-live-brief-${viewport}-${tema}.png`;
  test(`portal live-brief ${viewport} ${tema}`, async ({ page }) => {
