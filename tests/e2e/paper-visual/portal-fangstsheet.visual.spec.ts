...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/fangstsheet.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-fangstsheet-${viewport}-${tema}.png`;
  test(`portal fangstsheet ${viewport} ${tema}`, async ({ page }) => {
