...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-okonomi.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-okonomi-${viewport}-${tema}.png`;
  test(`admin okonomi ${viewport} ${tema}`, async ({ page }) => {
