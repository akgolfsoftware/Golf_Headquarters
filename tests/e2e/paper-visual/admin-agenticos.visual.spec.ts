...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-agenticos.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-agenticos-${viewport}-${tema}.png`;
  test(`admin agenticos ${viewport} ${tema}`, async ({ page }) => {
