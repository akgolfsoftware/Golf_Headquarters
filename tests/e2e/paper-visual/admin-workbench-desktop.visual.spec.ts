...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/workbench-desktop.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-workbench-desktop-${viewport}-${tema}.png`;
  test(`admin workbench-desktop ${viewport} ${tema}`, async ({ page }) => {
