...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-innstillinger.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-settings-${viewport}-${tema}.png`;
  test(`admin settings ${viewport} ${tema}`, async ({ page }) => {
