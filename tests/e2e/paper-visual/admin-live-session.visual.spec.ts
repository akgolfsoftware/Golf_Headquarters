...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-live-session.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-live-session-${viewport}-${tema}.png`;
  test(`admin live-session ${viewport} ${tema}`, async ({ page }) => {
