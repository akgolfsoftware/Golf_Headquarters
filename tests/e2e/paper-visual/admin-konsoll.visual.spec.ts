...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-konsoll-desktop.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-konsoll-${viewport}-${tema}.png`;
  test(`admin konsoll ${viewport} ${tema}`, async ({ page }) => {
