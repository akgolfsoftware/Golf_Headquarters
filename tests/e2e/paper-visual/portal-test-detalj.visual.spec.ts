...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-test-detalj.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-test-detalj-${viewport}-${tema}.png`;
  test(`portal test-detalj ${viewport} ${tema}`, async ({ page }) => {
