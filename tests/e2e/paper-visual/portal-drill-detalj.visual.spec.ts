...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-drill-detalj.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-drill-detalj-${viewport}-${tema}.png`;
  test(`portal drill-detalj ${viewport} ${tema}`, async ({ page }) => {
