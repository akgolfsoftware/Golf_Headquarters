...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase2/playerhq/playerhq-fys-plan.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `portal-fys-plan-${viewport}-${tema}.png`;
  test(`portal fys-plan ${viewport} ${tema}`, async ({ page }) => {
