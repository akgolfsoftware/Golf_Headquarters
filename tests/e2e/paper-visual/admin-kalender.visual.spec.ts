...
const FASIT = path.resolve(__dirname, "../../../designsystem/paper/fase1/agencyos-kalender.html");
...
for (const { viewport, tema } of CASES) {
  const snapshotName = `admin-kalender-${viewport}-${tema}.png`;
  test(`admin kalender ${viewport} ${tema}`, async ({ page }) => {
