...
**OPPDATERT 2026-07-03 — premisset under er foreldet.** Beslutningen fra 2026-06-04 (`public/design-handover/AK Golf HQ Design System/` som låst sannhet) er overtatt av at gjeldende design nå utvikles på nytt hos Claude Design. Se punkt 2D for hva som faktisk ble gjort. Tabellen og resten av FASE 2 under beskriver den GAMLE situasjonen — brukes kun som historikk, ikke som gjeldende plan.
...
4. **2D — Fiks brutte pekere: LØST 2026-07-03, deretter overtatt av Anders' beslutning samme dag.** `docs/STATUS-NÅ.md` pekte på en fil som ikke fantes — fjernet. Deretter avklarte Anders at HELE denne FASE 2-premissen er utdatert: gjeldende design utvikles nå på nytt hos Claude Design, og skal leveres som en fersk zip-handover. Derfor er FJERNET i samme runde: `.claude/rules/design-porting-gate.md` (hele gate-regelen), `docs/design-handover-2026-06-24/` og `docs/design-handoff-komplett/` (begge handover-mapper), samt alle pekere til dem i `CLAUDE.md` og `.claude/rules/arkitektur.md`. **FASE 2 sin tabell over er dermed foreldet** — verken 4.-juni- eller 24.-juni-kilden er lenger «låst sannhet». `docs/platform/screen-context/all-screens.md` (30 Fasit-linjer) og `docs/platform/AGENT-BRIEF.md` sitt gate-steg peker fortsatt på den nå slettede 24.-juni-mappen — ikke rettet ennå, avventer Anders' avklaring om disse skal tømmes eller stå til ny handover kommer.
5. **2E — UTGÅTT 2026-07-03:** `.design-review/claude-code-handoff/` + de to adversarial-skriptene er SLETTET (ikke flyttet) — gammel fasit, foreldet av ny Claude Design-handover (Anders' beslutning: alt design fra nåværende handover, aldri gammelt). Samtidig slettet: `public/design-handover/` fra disk (Drive-speilet «Final AK Golf HQ» beholder innholdet) + gitignore-linjen, og alle `public/design-handover/`-kommentarer i src/ omskrevet til `[historisk fasit, fjernet 2026-07-03]`. GJENSTÅR: `src/lib/portal-routes.ts` sine designPath-felter + `/admin/godkjenn-portal`-verktøyet peker fortsatt funksjonelt på slettede designfiler — krever Anders-beslutning (fjern verktøyet, eller behold som historisk mapping).
