...
export function WorkbenchMinKalender({ playerId, coachName, data }: { playerId: string; coachName: string; data: MinKalenderData }) {
  ...
  return <div className="wb-layout wb-min-layout">
    ...
    <main className="wb-main">
      <div className="wb-pills"><VisningPiller playerId={playerId} visning="min" uke={data.weekStart} maned={data.weekStart.slice(0, 7)} aar={data.weekStart.slice(0, 4)} scrollActive={false} /></div>
      <div className="wb-body wb-min-body">
        ...
        <section className="wb-calendar wb-min-calendar" aria-label="Min kalender">
          ...
          {data.days.map((day, index) => {
            ...
            return <div key={day.date} className="wb-calendar-day" data-mobile-selected={dayIndex === index}><div className="wb-day-heading"><button type="button" className="wb-min-day-switch" aria-label={`Neste dag etter ${DAYS[index]} ${Number(day.date.slice(8))}.`} onClick={() => { setDayIndex((index + 1) % data.days.length); setMobileInspectorOpen(false); }}>{DAYS[index]} <b>{Number(day.date.slice(8))}.</b></button></div><div className="wb-band"/><div className="wb-day-hours">{day.items.map((item) => {
              ...
              return <button key={item.id} type="button" className="wb-session" data-lag={item.pyramid ?? "TEK"} aria-pressed={selected?.id === item.id} style={style} onClick={() => { setSelectedId(item.id); setMobileInspectorOpen(true); }}><span>{item.title}</span><small>{formatTime(item.startMinute)}–{formatTime(item.startMinute + item.durationMinutes)}</small></button>;
    ...
    {selected ? <aside className="wb-mobile-summary wb-min-mobile" data-open={mobileInspectorOpen} aria-label="Valgt økt"><div className="wb-grip" aria-hidden/><Inspector item={selected} /></aside> : null}
  </div>;
}
