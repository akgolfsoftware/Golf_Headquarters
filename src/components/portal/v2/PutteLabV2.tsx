...
const KAT = {
  ctrl: TL.fill,
  ext: TL.warn,
  mind: TL.warn, // Paper: unngå lime som kategori på lys flate
} as const;

const GRESS = {
  ramme: `color-mix(in srgb, ${TL.fill} 62%, black)`,
  bunn: `color-mix(in srgb, ${TL.fill} 78%, black)`,
  feltLys: `color-mix(in srgb, ${TL.fill} 72%, white)`,
  feltMid: `color-mix(in srgb, ${TL.fill} 88%, white)`,
  feltMork: `color-mix(in srgb, ${TL.fill} 80%, black)`,
  kant: `color-mix(in srgb, ${TL.fill} 45%, white)`,
};
...
function Seg({ items, active, onPick }: { items: string[]; active: number; onPick: (i: number) => void }) {
  return (
    <div  data-paper-slug="playerhq-putte-lab" style={{ display: "inline-flex", flexWrap: "wrap", gap: 3, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 3 }}>
      {items.map((it, i) => {
        const on = i === active;
        return (
          <button
    ...
    >
      ...
      <g style={{ cursor: "grab" }} onPointerDown={handleSlopeDown}>
        ...
        <g transform={`translate(${kx},${ky}) rotate(${s.dir})`}>
          <circle r="15" fill={BLEKK} stroke={TL.fill} strokeWidth="2" />
          <path d="M0 7 L0 -6 M-4 -2 L0 -7 L4 -2" fill="none" stroke={TL.fill} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </g>
      </g>
      <text x={GCX} y={GCY + RING_R + 18} textAnchor="middle" fontFamily={TL.font.mono} fontSize="9" fontWeight="700" fill={KRITT} fillOpacity="0.6" letterSpacing="1">FALL ↓ {s.dir}°</text>

      <path d={prev} fill="none" stroke={BLEKK} strokeOpacity="0.8" strokeWidth="2.5" strokeDasharray="3 6" strokeLinecap="round" />
      <line x1={CX} y1={BALL_Y} x2={aimX} y2={HOLE_Y} stroke={TL.fill} strokeWidth="2.5" strokeDasharray="7 5" strokeLinecap="round" />
      ...
      <g opacity={Math.abs(aimX - ghost) > 2 ? 0.8 : 0}>
        ...
        <text x={ghost} y={HOLE_Y - 18} textAnchor="middle" fontFamily={TL.font.mono} fontSize="8" fontWeight="700" fill={KRITT} fillOpacity="0.7">IDEELT</text>
      </g>
      ...
      <path d={`M${CX} ${HOLE_Y - 44} L${CX + 22} ${HOLE_Y - 38} L${CX} ${HOLE_Y - 32} Z`} fill={TL.fill} />

      <g style={{ cursor: "ew-resize" }} transform={`translate(${aimX},${HOLE_Y})`} onPointerDown={handleAimDown}>
        <path d="M0 -26 L7 -16 L0 -6 L-7 -16 Z" fill={TL.fill} stroke={BLEKK} strokeWidth="1.5" />
        <text x="0" y="-30" textAnchor="middle" fontFamily={TL.font.mono} fontSize="8" fontWeight="800" fill={KRITT}>SIKT</text>
      </g>
      ...
      <circle
        ...
        strokeWidth="1"
      />
      <text x={CX} y={BALL_Y + 22} textAnchor="middle" fontFamily={TL.font.mono} fontSize="8" fontWeight="700" fill={KRITT} fillOpacity="0.7">BALL</text>
    </svg>
  );
}
...
function DirGreenen() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20, alignItems: "start" }}>
      <div>
        <div style={{ background: GRESS.ramme, border: `1px solid ${TL.hair}`, borderRadius: 12, overflow: "hidden" }}>
          <GnSvg s={s} ballPos={ballPos} onDragSlope={handleDragSlope} onDragAim={handleDragAim} />
        </div>
        <p style={{ margin: "10px 0 0", fontFamily: TL.font.mono, fontSize: 10, color: TL.mute, lineHeight: 1.7 }}>
          {result
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            ...
            <div key={r.l} style={{ background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 12 }}>
              <Caps size={9}>{r.l}</Caps>
              <div style={{ fontFamily: TL.font.mono, fontWeight: 700, fontSize: 20, lineHeight: 1.2, color: TL.text, marginTop: 3, fontVariantNumeric: "tabular-nums" }}>{r.v}</div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div style={{ position: "relative", height: 24, display: "flex", alignItems: "center" }}>
            <div style={{ position: "absolute", left: 0, right: 0, height: 7, borderRadius: 9999, background: TL.hair }} />
            {/* «Død i hullet»-sonen */}
            <div style={{ position: "absolute", left: `${lo}%`, width: `${hi - lo}%`, height: 7, borderRadius: 9999, background: `color-mix(in srgb, ${TL.ok} 30%, transparent)`, pointerEvents: "none" }} />
            <div style={{ position: "absolute", left: 0, width: `${((s.pace - 20) / 65) * 100}%`, height: 7, borderRadius: 9999, background: TL.fill, opacity: 0.85 }} />
            <span style={{ position: "absolute", left: `calc(${((s.pace - 20) / 65) * 100}% - 9px)`, width: 18, height: 18, borderRadius: 9999, background: TL.fill, border: `2px solid ${TL.elev}`, pointerEvents: "none" }} />
            <input
        ...
        {!result ? (
          ...
          <>
            <div style={{ borderRadius: 12, padding: 16, background: result.make ? "color-mix(in srgb, var(--tl-ok) 16%, var(--tl-scene))" : TL.dock, border: result.make ? "none" : `1px solid ${TL.hair}` }}>
...
function DirKjeden() {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 24, alignItems: "start" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 4 }}>
            {STAGE_KEYS.map(k => (
              <div key={k} style={k === worstKey ? { borderRadius: 12, padding: 12, background: `color-mix(in srgb, ${TL.warn} 9%, transparent)`, border: `1px solid color-mix(in srgb, ${TL.warn} 30%, transparent)` } : undefined}>
                <Glider
                ...
                />
                <p style={{ fontFamily: TL.font.mono, fontSize: 10, color: TL.mute, margin: "4px 0 0", lineHeight: 1.5 }}>{C.STAGE[k].blurb}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {PRESETS.map(p => (
              <button
                ...
                style={{ appearance: "none", cursor: "pointer", fontFamily: TL.font.sans, fontSize: 12, fontWeight: 600, padding: "7px 13px", borderRadius: 9999, background: TL.dim, border: `1px solid ${TL.hair}`, color: TL.text }}
              >
                {p.label}
              </button>
            ))}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ background: TL.fill, borderRadius: 16, padding: 22 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
            {gates.map(g => (
              <div key={g.key} style={{ display: "flex", alignItems: "center", gap: 12, ...(g.isWorst ? { borderRadius: 9, padding: 4, background: `color-mix(in srgb, ${TL.warn} 8%, transparent)` } : null) }}>
                <span style={{ fontFamily: TL.font.mono, fontSize: 11, fontWeight: 700, color: TL.mute, width: 108, flex: "none" }}>{g.label}</span>
                <div style={{ flex: 1, height: 7, borderRadius: 9999, background: TL.hair, overflow: "hidden" }}>
                  <div
                    style={{
                      ...
                      background: g.cls === "geom" ? TL.hair : g.cls === "flax" ? `color-mix(in srgb, ${TL.warn} 60%, transparent)` : g.isWorst ? TL.warn : TL.fill,
                      transition: `width 300ms ${TL.motion.ease}`,
                    }}
                  />
                </div>
                <span style={{ fontFamily: TL.font.mono, fontSize: 12, fontWeight: 700, width: 40, textAlign: "right", color: TL.text, fontVariantNumeric: "tabular-nums" }}>{Math.round(g.surv * 100)}%</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {worstKey && C.STAGE[worstKey as StageKey] && (() => {
          ...
          return (
            <div style={{ background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 16 }}>
              <Caps size={9} style={{ marginBottom: 6 }}>Svakeste ledd</Caps>
              <p style={{ fontFamily: TL.font.sans, fontSize: 13, lineHeight: 1.6, color: TL.text, margin: 0 }}>
...
function KontrollDial({ q }: { q: QVals }) {
  ...
  return (
    <svg viewBox={`0 0 ${VB} ${VB}`} role="img" aria-label="Kontroll-skive" style={{ width: "100%", height: "auto" }}>
      <circle cx={KCX} cy={KCY} r="196" fill="none" stroke={TL.warn} strokeWidth="3" strokeDasharray="3 9" opacity="0.55" />
      <text x={KCX} y="26" textAnchor="middle" fontFamily={TL.font.mono} fontSize="10" fontWeight="800" letterSpacing="1.5" fill={TL.warn}>FLAKS · UTENFOR KONTROLL</text>
      ...
      {stageArr.map(([k, lbl], i) => {
        ...
        return (
          <g key={k}>
            <path d={kontrollArc(r, a0, a1)} fill="none" stroke={TL.hair} strokeWidth="26" strokeLinecap="round" />
            <path d={kontrollArc(r, a0, fillEnd)} fill="none" stroke={TL.fill} strokeWidth="26" strokeLinecap="round" />
            <text x={lp.x.toFixed(1)} y={lp.y.toFixed(1)} textAnchor="middle" dominantBaseline="middle" fontFamily={TL.font.mono} fontSize="11" fontWeight="800" fill={TL.text}>{lbl.toUpperCase()}</text>
            <text x={lp.x.toFixed(1)} y={(lp.y + 14).toFixed(1)} textAnchor="middle" dominantBaseline="middle" fontFamily={TL.font.mono} fontSize="10" fontWeight="700" fill={TL.mute}>{Math.round(q[k] * 100)}</text>
          </g>
        );
      })}

      <circle cx={KCX} cy={KCY} r="96" fill={TL.fill} opacity="0.10" />
      <circle cx={KCX} cy={KCY} r="92" fill="none" stroke={TL.fill} strokeWidth="2.5" />
      <text x={KCX} y={KCY - 22} textAnchor="middle" fontFamily={TL.font.mono} fontSize="11" fontWeight="800" letterSpacing="2" fill={TL.mute} opacity="0.8">AKSEPT</text>
      <text x={KCX} y={KCY + 18} textAnchor="middle" fontFamily={TL.font.mono} fontSize="58" fontWeight="700" fill={TL.text} letterSpacing="-2">{proc}</text>
      <text x={KCX} y={KCY + 44} textAnchor="middle" fontFamily={TL.font.mono} fontSize="11" fontWeight="700" fill={TL.mute}>PROSESS-SCORE</text>
    </svg>
  );
}
  ...
  return (
    <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
      {([[KAT.ctrl, "I din kontroll"], [KAT.ext, "Flaks · utenfor"], [KAT.mind, "Mentalt"]] as [string, string][]).map(([farge, lbl]) => (
        <span key={lbl} style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: TL.font.mono, fontSize: 10, fontWeight: 700, letterSpacing: "0.05em", color: TL.mute }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div style={{ background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 12 }}>
            ...
            <div style={{ fontFamily: TL.font.mono, fontWeight: 700, fontSize: 32, lineHeight: 1.1, color: TL.fill, marginTop: 4, fontVariantNumeric: "tabular-nums" }}>{proc}</div>
            <div style={{ fontFamily: TL.font.mono, fontSize: 10, color: TL.mute }}>Snitt av leddene du styrer</div>
          </div>
          <div style={{ background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 12 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
            ...
            <div style={{ fontFamily: TL.font.mono, fontWeight: 700, fontSize: 32, lineHeight: 1.1, color: TL.text, marginTop: 4, fontVariantNumeric: "tabular-nums" }}>
              {Math.round(make * 100)}<small style={{ fontSize: 16, fontWeight: 600, color: TL.mute }}> %</small>
            </div>
            <div style={{ fontFamily: TL.font.mono, fontSize: 10, color: TL.mute }}>Avhenger også av flaks</div>
          </div>
        </div>

        <div style={{ background: TL.elev, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 16 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, gap: 10 }}>
            <Caps size={9}>Ti putter · samme prosess</Caps>
            {runResult ? (
              <span style={{ fontFamily: TL.font.mono, fontSize: 11, fontWeight: 700, color: TL.fill, fontVariantNumeric: "tabular-nums" }}>{makes}/10 i hull</span>
            ) : (
              <span style={{ fontFamily: TL.font.mono, fontSize: 11, color: TL.mute, fontVariantNumeric: "tabular-nums" }}>forventet {exp}/10</span>
            )}
          </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    height: 26,
                    borderRadius: 9999,
                    transition: `background 150ms ${TL.motion.ease}, border-color 150ms ${TL.motion.ease}`,
                    background: vist ? (runResult![i] ? TL.fill : `color-mix(in srgb, ${TL.danger} 20%, transparent)`) : TL.dim,
                    border: `2px solid ${vist ? (runResult![i] ? TL.fill : `color-mix(in srgb, ${TL.danger} 40%, transparent)`) : TL.hair}`,
                  }}
                />
              );
            })}
        ...
        {msg && (
          <div style={{ background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 12, padding: 16 }}>
            <div style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 14.5, letterSpacing: "-0.01em", color: TL.text, marginBottom: 4 }}>{msg.mt}</div>
            <p style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, lineHeight: 1.6, margin: 0 }}>{msg.mb}</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div>
        ...
        <div style={{ marginTop: 10 }}>
          <Tittel>
            Putten er <em style={{ fontStyle: "italic", color: TL.fill }}>seks ledd</em> — break er bare ett.
          </Tittel>
        </div>
        <p style={{ margin: "12px 0 0", maxWidth: "60ch", fontFamily: TL.font.sans, fontSize: 13.5, lineHeight: 1.6, color: TL.mute }}>
          Greenhastighet og fall avgjør <strong style={{ color: TL.text }}>hvor mye putten brekker</strong> — kjernen. Men en putt går gjennom seks ledd: lese, sikte, starte ballen, dosere lengden, og til slutt det du ikke styrer. Tre verktøy, tre måter å tenke på den samme kjeden.
        </p>
      </div>
      ...
      <Kort pad="0">
        <div style={{ display: "flex", flexWrap: "wrap" }}>
          {MODEL_STEPS.map(([n, t, dd, c]) => (
            <div key={n} style={{ flex: 1, minWidth: 150, padding: "12px 14px", borderLeft: `3px solid ${KAT[c as keyof typeof KAT]}`, borderRight: `1px solid ${TL.hair}` }}>
              <span style={{ fontFamily: TL.font.mono, fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: TL.mute, display: "block" }}>{n}</span>
              <span style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 14.5, letterSpacing: "-0.01em", color: TL.text, display: "block", marginTop: 2 }}>{t}</span>
              <span style={{ fontFamily: TL.font.sans, fontSize: 11.5, lineHeight: 1.45, color: TL.mute, display: "block", marginTop: 4 }}>{dd}</span>
            </div>
          ))}
        </div>
      </Kort>
          ...
          return (
            <button
              ...
              style={{
                ...
                textAlign: "left",
                borderRadius: 13,
                background: on ? TL.dim : TL.elev,
                border: `1px solid ${on ? TL.fill : TL.hair}`,
              }}
            >
              <span style={{ fontFamily: TL.font.mono, fontSize: 9.5, fontWeight: 700, letterSpacing: "0.12em", color: on ? TL.fill : TL.mute }}>{n}</span>
              <span style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 16, letterSpacing: "-0.01em", color: TL.text }}>{tt}</span>
              <span style={{ fontFamily: TL.font.sans, fontSize: 11.5, lineHeight: 1.4, color: TL.mute }}>{dd}</span>
            </button>
          );
        })}
      </div>
      ...
      <p style={{ margin: 0, fontFamily: TL.font.mono, fontSize: 10, color: TL.mute, lineHeight: 1.7 }}>
        {activeDir === "gn" && <><strong style={{ color: TL.text }}>Greenen:</strong> dra siktepunktet, doser farten, slå. Ballen ruller med break-at-speed-fysikken fra referansetabellen — bom på sikte eller lengde og du ser nøyaktig hvilket ledd som kostet putten.</>}
        {activeDir === "kj" && <><strong style={{ color: TL.text }}>Kjeden:</strong> make-% er et produkt, ikke en sum. Geometrien setter taket; hvert ledd du utfører dårlig ganger ned sjansen. Det svakeste leddet er der du henter mest. Flaks er taket du aldri kommer forbi.</>}
        {activeDir === "kn" && <><strong style={{ color: TL.text }}>Kontroll:</strong> prosess-scoren måler kun det du styrer. Make-raten avhenger også av flaks. Slå ti putter med samme prosess og se sprikningen — derfor dømmer du deg selv på prosessen, ikke på om den siste putten gikk i.</>}
      </p>
    </div>
  );
}
