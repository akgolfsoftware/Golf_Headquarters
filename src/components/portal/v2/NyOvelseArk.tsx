...
/**
 * OvelseSkjemaFelter — feltene alene, uten BunnArk-innpakning. Utskilt
 * (2026-07-13) så «Ny øvelse» kan vises INNE i et allerede åpent ark
 * (Workbench økt-komponisten) uten modal-i-modal (forbudt anti-mønster,
 * se mesterens-monstre.md) — kalleren bytter bare innholdet i sitt eget
 * BunnArk fremfor å stable et nytt oppå.
 */
export function OvelseSkjemaFelter({ defaultAkse, onLagret, onAvbryt, onLagrerChange, lagreLabel = "Lagre øvelse" }: {
  defaultAkse?: AkseKey;
  /** Kalles ved suksess med den nyopprettede øvelsen. */
  onLagret: (ovelse: { id: string; name: string }) => void;
  onAvbryt: () => void;
  /** Lar kalleren speile lagrer-tilstanden (f.eks. i sin egen BunnArk sin `laast`-prop). */
  onLagrerChange?: (lagrer: boolean) => void;
  lagreLabel?: string;
}) {
  ...
  const settLagrer = (v: boolean) => { setLagrer(v); onLagrerChange?.(v); };
  ...
  const lagre = async () => {
    ...
    settLagrer(false);
    if (res.ok && res.id) {
      onLagret({ id: res.id, name: res.name ?? navn.trim() });
    } else {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 4 }}>
        <Knapp ghost onClick={onAvbryt} disabled={lagrer}>Avbryt</Knapp>
...
export function NyOvelseArk({ defaultAkse, onLukk, onOpprettet }: {
  ...
  return (
    <BunnArk tittel="Legg til øvelse" under="Lagres i øvelsesbanken din — coach ser den også." onLukk={onLukk} laast={lagrer}>
      <OvelseSkjemaFelter
        defaultAkse={defaultAkse}
        onLagrerChange={setLagrer}
        onAvbryt={onLukk}
        onLagret={(ovelse) => { onOpprettet(ovelse); onLukk(); }}
      />
    </BunnArk>
  );
}
