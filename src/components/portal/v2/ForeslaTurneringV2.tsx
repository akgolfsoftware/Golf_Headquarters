...
import { Caps, Kort, CTAPill, StatusPill, InnsiktChip, TomTilstand } from "@/components/v2";
    ...
    <Link href={t.href} style={{ textDecoration: "none", color: "inherit", display: "block" }}>
      <Kort hover>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          >
            <span style={{ display: "block", fontFamily: TL.font.mono, fontSize: 17, fontWeight: 700, color: TL.text, fontVariantNumeric: "tabular-nums", lineHeight: 1 }}>
              {t.day}
            </span>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <span style={{ fontFamily: TL.font.mono, fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: TL.mute, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 5, padding: "3px 7px" }}>
                {t.badge}
              </span>
            ...
            {(t.venue || t.meta.length > 0) && (
              <div style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap", fontFamily: TL.font.mono, fontSize: 10, fontWeight: 700, color: TL.mute, marginTop: 5 }}>
                {t.venue && (
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                    <Icon name="map-pin" size={10} style={{ color: TL.mute }} />
                    {t.venue}
                  </span>
                )}
                {t.meta.map((m, i) => (
          ...
          <Icon name="chevron-right" size={14} style={{ color: TL.mute, flex: "none", marginTop: 4 }} />
        </div>
        ...
        <div style={{ marginTop: 12, borderRadius: 12, background: TL.dock, border: `1px solid ${TL.hair}`, padding: "10px 12px" }}>
          ...
          <p style={{ fontFamily: TL.font.sans, fontSize: 12.5, color: TL.mute, lineHeight: 1.55, margin: "5px 0 0" }}>{t.why}</p>
        </div>
      </Kort>
    </Link>
  );
...
export function ForeslaTurneringV2({ data }: { data: ForeslaTurneringV2Data }) {
  ...
  return (
    <div data-paper-wave-g="foreslaturnering" data-paper-pattern style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 720, margin: "0 auto", width: "100%" }}>
      <div>
        <div data-paper-pattern-topp>
          <h1 style={{ margin: 0, fontFamily: TL.font.sans, fontSize: 17, fontWeight: 600, color: TL.text }}>Foreslå turnering</h1>
          <span style={{ display: "block", fontFamily: TL.font.mono, fontSize: 10.5, color: TL.mute, marginTop: 2 }}>Turnering</span>
        </div>
      ...
      <InnsiktChip>
        {catalogCount > 0 ? (
          <>
            Vurdert mot <span style={{ color: TL.text, fontWeight: 600 }}>{catalogCount} kommende turneringer</span>
            {" · "}HCP <span style={{ color: TL.text, fontWeight: 600 }}>{hcpLabel}</span>
          </>
        ) : (
