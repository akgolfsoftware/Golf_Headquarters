 ...
 * med ærlige tellere) · vanskelighet-pills · anlegg-pills · drill-kort med
 * venstre akse-farge-kant + akse-eyebrow + tittel + meta + CS-koblings-badge.
 * Klikk på et kort → /portal/drills/[id].
 *
 * Ingen stjerne-rating: databasen har ingen aggregert rating, og falske tall
 * er forbudt. Resten av kort-anatomien følger fasiten.
 */
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          {drill.difficulty && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-2 py-0.5 font-mono text-[9px] font-extrabold uppercase tracking-[0.08em] text-muted-foreground">
              <span className={cn("h-1.5 w-1.5 rounded-full", AXIS_DOT[drill.axis])} aria-hidden />
...
export function DrillLibrary({ drills }: { drills: DrillCard[] }) {
  ...
  // Hvilke akser/vanskelighet/anlegg finnes faktisk — skjul tomme valg.
  ...
  const visible = useMemo(() => {
    ...
    return drills.filter((d) => {
      ...
      if (fas !== "alle" && !d.fasilitet.includes(fas as never)) return false;
  ...
  // søk/vanskelighet/anlegg slik at tallene matcher det man faktisk kan se).
