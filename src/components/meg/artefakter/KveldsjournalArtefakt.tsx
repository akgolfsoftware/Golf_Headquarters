...
function StatKort({ tall, label }: { tall: number; label: string }) {
  return (
    <div style={{ flex: 1, background: T.panel, border: `1px solid ${T.border}`, borderRadius: T.rTag, padding: 12 }}>
      <div style={{ fontFamily: T.mono, fontSize: 24, fontWeight: 600, lineHeight: 1.1, color: T.fg }}>{tall}</div>
      <div style={{ fontFamily: T.mono, fontSize: 9.5, letterSpacing: "0.07em", textTransform: "uppercase", color: T.mut, marginTop: 4 }}>
        {label}
      </div>
    </div>
  );
...
export function KveldsjournalArtefakt({
  ...
  return (
    <div data-od-id="panel-kveldsjournal" style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <h2 style={{ margin: "0 0 8px", fontFamily: T.disp, fontSize: 13, fontWeight: 600, color: T.fg }}>Lukket i dag</h2>
      {lukketIDag.length === 0 ? (
        <p style={{ fontFamily: T.ui, fontSize: 13, color: T.mut, marginBottom: 16 }}>Ingenting lukket i dag ennå.</p>
      ) : (
        <ul style={{ listStyle: "none", margin: "0 0 16px", padding: 0 }}>
          {lukketIDag.map((l) => (
            <li key={l.id} style={{ display: "flex", gap: 8, alignItems: "baseline", padding: "3px 0", fontSize: 13, fontFamily: T.ui, color: T.fg }}>
              <span style={{ color: T.up, fontFamily: T.mono, flex: "none" }}>✓</span>
              <button
                type="button"
                onClick={() => l.sakId && onVelgSak(l.sakId)}
                disabled={!l.sakId}
                data-od-id={`cta-journal-lukket-${l.id}`}
                style={{ background: "none", border: 0, padding: 0, color: T.fg, cursor: l.sakId ? "pointer" : "default", fontFamily: T.ui, fontSize: 13, textAlign: "left" }}
              >
                {l.handling}
              </button>
              <span style={{ marginLeft: "auto", flex: "none", fontFamily: T.mono, fontSize: 11, color: T.mut }}>{klokke(l.tidspunkt)}</span>
            </li>
          ))}
        </ul>
      )}

      <h2 style={{ margin: "0 0 8px", fontFamily: T.disp, fontSize: 13, fontWeight: 600, color: T.fg }}>Restanse til i morgen</h2>
      {restanse.length === 0 ? (
        <p style={{ fontFamily: T.ui, fontSize: 13, color: T.mut, marginBottom: 16 }}>Ingen restanse — køen er tom.</p>
      ) : (
        <ul style={{ listStyle: "none", margin: "0 0 16px", padding: 0 }}>
          {restanse.map((s) => (
            <li key={s.id} style={{ display: "flex", gap: 8, padding: "3px 0", fontSize: 13, fontFamily: T.ui, color: T.fg }}>
              <span style={{ color: T.mut, flex: "none" }}>→</span>
              <button
                type="button"
                onClick={() => onVelgSak(s.id)}
                data-od-id={`cta-journal-restanse-${s.id}`}
                style={{ background: "none", border: 0, padding: 0, color: T.fg, textDecoration: "underline", cursor: "pointer", fontFamily: T.ui, fontSize: 13 }}
              >
      ...
      <h2 style={{ margin: "0 0 8px", fontFamily: T.disp, fontSize: 13, fontWeight: 600, color: T.fg }}>Fanget i dag</h2>
      {fangetIDag.length === 0 ? (
        <p style={{ fontFamily: T.ui, fontSize: 13, color: T.mut }}>Ingenting fanget i dag.</p>
      ) : (
