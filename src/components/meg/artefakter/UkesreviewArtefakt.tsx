...
function KpiKort({ tall, label, delta }: { tall: string; label: string; delta: string | null }) {
  return (
    <div style={{ flex: 1, background: TL.elev, border: `1px solid ${TL.hair}`, borderRadius: TL.radius.row, padding: "12px 14px" }}>
      <div style={{ fontFamily: TL.font.mono, fontSize: 28, fontWeight: 600, lineHeight: 1.1, color: TL.text }}>{tall}</div>
...
export function UkesreviewArtefakt({ data }: { data: UkesreviewData }) {
  ...
  return (
    <div data-od-id="panel-ukesreview" style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <div style={{ fontFamily: TL.font.mono, fontSize: 11, color: TL.mute, marginBottom: 12 }}>{periodeLabel}</div>
      ...
      <h2 style={{ margin: "0 0 8px", fontFamily: TL.font.sans, fontSize: 13, fontWeight: 600, color: TL.text }}>AI-kostnad denne uken</h2>
      {aiKost.antallKall === 0 ? (
        <p style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, marginBottom: 16 }}>Ingen AI-kostnad registrert for Jarvis denne uken.</p>
      ) : (
        <p style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.text, marginBottom: 16 }}>
          <span style={{ fontFamily: TL.font.mono }}>
            {(aiKost.inputTokens + aiKost.outputTokens).toLocaleString("nb-NO")} tokens
          </span>{" "}
          over <span style={{ fontFamily: TL.font.mono }}>{aiKost.antallKall}</span> kall
          {aiKost.costUsd != null && (
            <>
              {" · "}
              <span style={{ fontFamily: TL.font.mono }}>${aiKost.costUsd.toFixed(2)}</span>
            </>
          )}
        </p>
      )}

      <h2 style={{ margin: "0 0 8px", fontFamily: TL.font.sans, fontSize: 13, fontWeight: 600, color: TL.text }}>Saker per kanal</h2>
      {sakerPerKanal.length === 0 ? (
        <p style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, marginBottom: 16 }}>Ingen saker mottatt denne uken.</p>
      ) : (
        <div data-od-id="panel-kanalfordeling" style={{ marginBottom: 16 }}>
          {sakerPerKanal.map(({ kanal, antall }) => (
            <div key={kanal} style={{ display: "grid", gridTemplateColumns: "88px 1fr 32px", gap: 8, alignItems: "center", padding: "3px 0", fontSize: 12.5, fontFamily: TL.font.sans, color: TL.text }}>
              <span>{KANAL_LABEL[kanal]}</span>
              <span style={{ height: 8, background: TL.dock, borderRadius: TL.radius.pill, overflow: "hidden" }}>
                <span style={{ display: "block", height: "100%", width: `${(antall / maksKanal) * 100}%`, background: TL.viz.target, borderRadius: TL.radius.pill }} />
              </span>
              <span style={{ fontFamily: TL.font.mono, fontSize: 11, color: TL.mute, textAlign: "right", fontVariantNumeric: "tabular-nums" }}>{antall}</span>
            </div>
          ))}
        </div>
      )}

      <h2 style={{ margin: "0 0 8px", fontFamily: TL.font.sans, fontSize: 13, fontWeight: 600, color: TL.text }}>Kalenderavvik fanget</h2>
      <div style={{ display: "flex", gap: 8, alignItems: "flex-start", border: `1px dashed ${TL.hair}`, borderRadius: TL.radius.row, padding: "8px 10px" }}>
        <Icon name="shield-check" size={14} strokeWidth={1.8} style={{ color: TL.mute, flex: "none", marginTop: 1 }} />
