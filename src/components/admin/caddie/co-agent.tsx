 ...
 *
 * Bygget med DS-tokens (ingen hardkodet hex) + lucide (ingen emoji). Server
 * Component — all interaktivitet er bevisst statisk inntil logikk kobles på (fase 3).
 */
...
const sourceIcon: Record<DraftSource["icon"], LucideIcon> = {
  "file-text": FileText,
  layers: Layers,
...
const fleetIcon: Record<FleetAgent["icon"], LucideIcon> = {
  layers: Layers,
  mail: Mail,
  lightbulb: Lightbulb,
...
const statusClass: Record<CoAgentStatus, { wrap: string; dot: string; label: string }> = {
...
const accuracyFillClass: Record<FleetAgent["accuracyTone"], string> = {
  ok: "bg-success",
  warn: "bg-warning",
  bad: "bg-destructive",
};
...
function SectionHead({
  ...
  return (
    <div className="mb-3.5 mt-8 flex items-center gap-3 first:mt-0">
...
// ── SECTION 2 — UTKAST → GODKJENNING ────────────────────────────
function DraftPanel({ draft }: { draft: CoAgentDraft }) {
  if (!draft) {
    return (
      <div className="rounded-2xl border border-border bg-card">
        <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <p className="mt-1 max-w-sm text-sm text-muted-foreground">
            Når en co-agent foreslår en endring, dukker den opp her med kildebruk, diff og
            tre valg: godkjenn, rediger eller avvis.
          </p>
        </div>
      </div>
    );
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card">
      <div className="grid grid-cols-1 lg:grid-cols-[340px_1fr_280px]">
        ...
        <div className="flex flex-col gap-3 border-border p-6 lg:border-l">
          ...
          <button
            type="button"
            className="flex h-11 items-center justify-center gap-2 rounded-lg border border-border bg-card text-sm font-bold tracking-[-0.005em] text-foreground transition-colors hover:bg-secondary"
          >
            <Pencil className="h-3.5 w-3.5" strokeWidth={2} aria-hidden /> Rediger først
          </button>
          <button
            type="button"
            className="flex h-11 items-center justify-center gap-2 rounded-lg border border-destructive/20 bg-card text-sm font-bold tracking-[-0.005em] text-destructive transition-colors hover:bg-destructive/5"
          >
...
function FleetPanel({ fleet, summary }: { fleet: FleetAgent[]; summary: FleetSummary }) {
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  s.tone === "live" ? "text-success" : s.tone === "draft" ? "text-warning" : "text-foreground",
                )}
              >
                {s.value}
              </span>
                    ...
                    <td>
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="flex min-w-0 flex-col leading-tight">
                          <span className="truncate text-[13px] font-bold text-foreground">{a.name}</span>
                    ...
                    <td>
                      <div className="flex min-w-[100px] flex-col gap-1">
                        ...
                        <div className="h-[5px] overflow-hidden rounded-full bg-secondary">
                          <div
                            className={cn("h-full rounded-full", accuracyFillClass[a.accuracyTone])}
...
function AuditRowCells({ row }: { row: AuditRow }) {
  return (
    <>
      ...
      <div className="flex items-center border-b border-border py-3 pl-3">
        <span
          className={cn(
            ...
            row.actor === "agent" ? "bg-accent shadow-[0_0_0_2px_hsl(var(--accent)/0.3)]" : "bg-info",
          )}
        />
      </div>
...
// ── Hovedkomponent ──────────────────────────────────────────────
export function CoAgent({ coachFirstName, dateLabel, timeLabel, draft, fleet, fleetSummary, audit }: CoAgentProps) {
  return (
    <div className="mx-auto max-w-[1200px]">
      ...
      <div className="mb-6">
        ...
        <h1 className="mt-2 font-display text-[30px] font-bold leading-[1.05] tracking-[-0.025em] text-foreground">
          God dag, {coachFirstName} — <em className="font-normal italic text-primary">caddien</em> jobber for deg.
        </h1>
      ...
      <p className="mt-4 text-center font-mono text-[10px] tracking-[0.04em] text-muted-foreground">
        ...
        <Link href="/admin/agencyos/caddie/aktivitet" className="text-primary underline-offset-2 hover:underline">
