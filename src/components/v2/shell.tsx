...
/* ---------- DS2: tema (lys default på app-flater, mørk via bryter — Anders 25. jul) ---------- */
...
function IkonRailNav({ aktiv, nav, mer, navn, avatarUrl, erAgency }: Required<Pick<V2ShellProps, "nav" | "navn">> & { aktiv?: string; mer?: V2NavGruppe[]; avatarUrl?: string | null; erAgency?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* TEMA-FASIT (25. jul — opphever B28): lys er standard på ALLE v2-flater,
          mørk er et bevisst valg per bruker — derfor vises bryteren overalt
          (også PlayerHQ/Forelder), ikke bare i AgencyOS. */}
      <TemaRailKnapp />
...
export function V2Shell({ aktiv, nav = PLAYERHQ_NAV, mer, navn = "Øyvind Rohjan", avatarUrl, vekslerData, children }: V2ShellProps) {
  ...
  // skjema-primitiver og scrollbars matcher. SSR er lys (ny standard 25. jul);
  ...
  // TEMA-FASIT (Anders, 25. jul — OPPHEVER B28): ALLE v2-flater (PlayerHQ,
  // AgencyOS, Forelder) er lys som standard med bryter til mørk. Mørk skjerm
  // er vanskelig å lese utendørs i sollys. `data-v2-tema` er ETT delt
  // attributt på <html> (samme cookie), og Next-navigasjon mellom flatene er
  // client-side (samme dokument), så attributtet synkes ved rute-veksling:
  // mørk KUN hvis cookien sier dark — ellers alltid lys.
  useEffect(() => {
