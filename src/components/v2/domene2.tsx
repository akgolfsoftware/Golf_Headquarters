...
/* AK Golf HQ v2 — DOMENE-KOMPONENTER 2 (retning C «Presis»).
   ...
   opp/ned = TL.ok/TL.danger (ALDRI lime), lime kun aksent/hero, klarspråk
...
export function Bit({ icon, monoTekst, children }: BitProps) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: monoTekst ? TL.font.mono : TL.font.sans, fontSize: monoTekst ? 9 : 11, fontWeight: monoTekst ? 700 : 600, color: TL.mute, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 5, padding: "3px 7px", whiteSpace: "nowrap" }}>
      {icon && <Icon name={icon} size={11} style={{ color: TL.mute }} />}{children}
    </span>
  );
}
...
export function LFaseBadge({ fase = "L3", label = null, kompakt = false }: LFaseBadgeProps) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 7, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 5, padding: "3px 8px" }}>
      <span style={{ ...mono(9.5, TL.text) }}>L{n}</span>
      <span style={{ display: "inline-flex", gap: 2.5 }}>
        {[1, 2, 3, 4, 5].map((i) => <span key={i} style={{ width: 4, height: 4, borderRadius: 9999, background: i <= n ? TL.mute : TL.hair }} />)}
      </span>
      {!kompakt && <span style={{ fontFamily: TL.font.sans, fontSize: 10.5, color: TL.mute }}>{label || LFASE_NAVN[n]}</span>}
    </span>
  );
}
...
export function BenchmarkBadge({ nivaa = "CS90", percentil = 72, sammenheng = "av spillere i din kategori", label = "Klubbhastighet" }: BenchmarkBadgeProps) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 12, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: TL.radius.row, padding: "10px 14px" }}>
      <span style={{ width: 48, height: 40, borderRadius: 10, background: TL.dim, border: `1px solid ${TL.hair}`, display: "inline-flex", alignItems: "center", justifyContent: "center", ...mono(13), flex: "none" }}>{nivaa || "—"}</span>
      <div style={{ minWidth: 0 }}>
        <Caps size={8.5}>{label}</Caps>
        <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
          <span style={{ ...mono(15) }}>{percentil == null ? "—" : `${percentil}.`}</span>
          <span style={{ fontFamily: TL.font.sans, fontSize: 10.5, color: TL.mute }}>percentil {sammenheng}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <Kort pad="15px 17px">
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginTop: 12 }}>
        <span style={{ ...mono(20, TL.mute, 600) }}>{foer == null ? "—" : foer}</span>
        <Icon name="arrow-right" size={15} style={{ color: TL.mute, alignSelf: "center" }} />
        <span style={{ ...mono(32), lineHeight: 0.95 }}>{etter == null ? "—" : etter}<span style={{ fontSize: 13, color: TL.mute, marginLeft: 4 }}>{enhet}</span></span>
      ...
      {periode && <span style={{ fontFamily: TL.font.sans, fontSize: 11, color: TL.mute, display: "block", marginTop: 9 }}>{periode}</span>}
    </Kort>
  );
}
...
export function FleksMerke({ fleks = true, tekst = null }: FleksMerkeProps) {
  const c = fleks ? TL.viz.target : TL.mute;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: TL.font.mono, fontSize: 9, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: c, background: `color-mix(in srgb,${c} 10%,transparent)`, borderRadius: 9999, padding: "4px 9px" }}>
...
export function LiveStatus({ tekst = "Økt pågår", tone = "down", tid = null }: LiveStatusProps) {
  const c: string = { down: TL.danger, up: TL.ok, lime: TL.fill, warn: TL.warn, info: TL.viz.target }[tone] || TL.danger;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontFamily: TL.font.sans, fontSize: 12.5, fontWeight: 600, color: TL.text }}>{tekst}</span>
      {tid && <span style={{ ...mono(12, TL.mute, 600) }}>{tid}</span>}
    </span>
  );
}
        ...
        return (
          <div key={t} style={{ display: "flex", gap: 12 }}>
            <span style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 16, flex: "none" }}>
              <span style={{ width: er ? 14 : 8, height: er ? 14 : 8, borderRadius: 9999, marginTop: er ? 2 : 5, flex: "none", background: er ? TL.fill : naadd ? TL.mute : "transparent", border: er ? `2px solid ${TL.elev}` : `1.5px solid ${naadd ? TL.mute : TL.hair}` }} />
              {i < trinn.length - 1 && <span style={{ width: 1, flex: 1, minHeight: 14, background: TL.hair }} />}
            </span>
    ...
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 12, background: TL.elev, border: `1px solid ${TL.hair}`, borderRadius: TL.radius.row + 4, padding: "13px 16px", cursor: onClick ? "pointer" : "default" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 3 }}>
          {sub && <span style={{ fontFamily: TL.font.sans, fontSize: 11, color: TL.mute }}>{sub}</span>}
          {frist && !ferdig && <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontFamily: TL.font.sans, fontSize: 11, color: status === "forfalt" ? TL.danger : TL.mute }}><Icon name="clock" size={11} />{frist}</span>}
        </div>
      </div>
  ...
  return (
    <Kort pad="14px 16px">
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
        <Caps>SG-splitt</Caps>
        <span style={{ fontFamily: TL.font.mono, fontSize: 9, color: TL.mute }}>mot {baseline}</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${kategorier.length}, 1fr)`, gap: 8 }}>
        {kategorier.map((x, i) => {
          const c = x.sg == null ? TL.mute : x.sg >= 0 ? TL.ok : TL.danger;
          return (
            <div key={x.k} style={{ textAlign: "center", padding: "8px 4px", borderLeft: i === 0 ? "none" : `1px solid ${TL.hair}` }}>
              <span style={{ ...mono(16, c), display: "block", lineHeight: 1 }}>{x.sg == null ? "—" : fmtSg(x.sg)}</span>
              <span style={{ fontFamily: TL.font.mono, fontSize: 8, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: TL.mute, display: "block", marginTop: 6 }}>{KAT_NAVN[x.k] || x.k}</span>
            </div>
          );
        })}
      </div>
...
export function VelvaereKort({
  ...
  const farge = (v: number | null | undefined) => (v == null ? TL.mute : v <= 2 ? TL.danger : v === 3 ? TL.warn : TL.ok);
  return (
    <Kort eyebrow="Velvære" action={<span style={{ fontFamily: TL.font.mono, fontSize: 9, color: TL.mute }}>{dato}</span>} pad="15px 17px">
      {rader.map((r, i) => {
        const v = verdier ? verdier[r.key] : null, c = farge(v);
        return (
          <div key={r.key} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i === rader.length - 1 ? "none" : `1px solid ${TL.hair}` }}>
            <Icon name={r.ikon} size={14} style={{ color: TL.mute, flex: "none" }} />
            <span style={{ flex: 1, fontFamily: TL.font.sans, fontSize: 12.5, color: TL.mute }}>{r.l}</span>
            <span style={{ display: "inline-flex", gap: 4 }}>
              {[1, 2, 3, 4, 5].map((d) => <span key={d} style={{ width: 8, height: 8, borderRadius: 9999, background: v != null && d <= v ? c : TL.hair }} />)}
            </span>
            <span style={{ width: 30, textAlign: "right", ...mono(12, v == null ? TL.mute : TL.text) }}>{v == null ? "—" : `${v}/5`}</span>
          </div>
        );
      })}
...
export function TidsVelger({
  ...
  const knapp: CSSProperties = { width: 34, height: 34, borderRadius: 9999, background: TL.dim, border: `1px solid ${TL.hair}`, display: "inline-flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flex: "none" };
  ...
  return (
    <Kort pad="15px 17px">
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 18, margin: "14px 0 4px" }}>
        <span style={knapp} onClick={() => skift(-15)}><Icon name="minus" size={14} style={{ color: TL.mute }} /></span>
        <span style={{ ...mono(40), letterSpacing: "-0.02em", lineHeight: 1 }}>{tid}</span>
        <span style={knapp} onClick={() => skift(15)}><Icon name="plus" size={14} style={{ color: TL.mute }} /></span>
      </div>
      <div style={{ fontFamily: TL.font.sans, fontSize: 10.5, color: TL.mute, textAlign: "center", marginBottom: 12 }}>± 15 min</div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
        {forslag.map((t) => {
          const on = t === tid;
          return (
            <button key={t} onClick={() => velg(t)} style={{ appearance: "none", cursor: "pointer", fontFamily: TL.font.mono, fontSize: 11.5, fontWeight: 700, fontVariantNumeric: "tabular-nums", padding: "6px 12px", borderRadius: 9999, color: on ? TL.onFill : TL.mute, background: on ? TL.fill : TL.dock, border: `1px solid ${on ? "transparent" : TL.hair}` }}>{t}</button>
          );
        })}
      </div>
    </Kort>
    ...
    <Kort tint pad="16px 18px">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 11 }}>
          ...
          <span>
            <span style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 16, color: TL.text, display: "block" }}>{barn}</span>
            <span style={{ fontFamily: TL.font.sans, fontSize: 11, color: TL.mute }}>Slik gikk uken</span>
          </span>
        </span>
        <span style={{ fontFamily: TL.font.mono, fontSize: 10, fontWeight: 700, color: TL.mute, background: TL.dock, border: `1px solid ${TL.hair}`, borderRadius: 5, padding: "4px 8px" }}>{uke}</span>
      </div>
      <p style={{ fontFamily: TL.font.sans, fontSize: 12.5, color: TL.mute, lineHeight: 1.6, margin: "12px 0 0" }}>{oppsummering}</p>
      ...
      {fraCoach && (
        <div style={{ display: "flex", gap: 8, alignItems: "flex-start", marginTop: 12, padding: "10px 12px", borderRadius: 12, background: TL.dock, border: `1px solid ${TL.hair}` }}>
          <Icon name="message-circle" size={13} style={{ color: TL.fill, flex: "none", marginTop: 1 }} />
          <span style={{ fontFamily: TL.font.sans, fontSize: 12, color: TL.mute, lineHeight: 1.5 }}>«{fraCoach}» <span style={{ color: TL.mute }}>— {coach}</span></span>
        </div>
      )}
    </Kort>
  );
