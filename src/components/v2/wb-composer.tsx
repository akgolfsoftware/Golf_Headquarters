...
export function PalettSok({ value = "", onChange, placeholder = "Søk økt, drill, mal…" }: PalettSokProps) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, height: 38, padding: "0 12px", borderRadius: 11, background: TL.dock, border: `1px solid ${TL.hair}`, minWidth: 0 }}>
      ...
      <input value={value} onChange={(e) => onChange && onChange(e.target.value)} placeholder={placeholder}
        style={{ flex: 1, minWidth: 0, appearance: "none", background: "transparent", border: "none", outline: "none", color: TL.text, fontFamily: TL.font.sans, fontSize: 12.5 }} />
      {value ? <button onClick={() => onChange && onChange("")} title="Tøm" style={{ appearance: "none", cursor: "pointer", background: "transparent", border: "none", color: TL.mute, display: "inline-flex", flex: "none", padding: 0 }}><Icon name="x" size={13} /></button> : null}
    </div>
  );
}
