 ...
 * Portet 1:1 fra Claude Design-prosjektets components/data/Pyramid.jsx
 * (hentet via DesignSync 2026-07-08).
 */

function usePyramidMotion(rowCount: number) {
  const reduceMotion =
  ...
  React.useEffect(() => {
    ...
    // All setState i rAF — aldri synkront i effekten (react-hooks/set-state-in-effect).
...
export type PyramidAxis = {
  /** FYS | TEK | SLAG | SPILL | TURN. */
  axis: string;
  /** Faktisk verdi (0–max). */
  value: number;
  /** Valgfri plan-/målverdi — tegnes som markørlinje. */
  plan?: number;
};

export type PyramidProps = {
  /** Akser topp→bunn (apex→base). Default TURN…FYS. */
  data?: PyramidAxis[];
  /** Fremhev én akse i lime. */
  activeAxis?: string;
  max?: number;
  showValues?: boolean;
  compact?: boolean;
...
const DEFAULT_DATA: PyramidAxis[] = [
  { axis: "TURN", value: 40 },
  { axis: "SPILL", value: 62 },
  { axis: "SLAG", value: 71 },
  { axis: "TEK", value: 84 },
...
export function Pyramid({
  ...
  const [hover, setHover] = React.useState<{ row: number; x: number } | null>(null);
  return (
    <div className={className} style={{ display: "flex", flexDirection: "column", gap: rowGap, ...style }}>
      {data.map((d, rowIdx) => {
        ...
        return (
          <div
            ...
            onMouseMove={(e) => {
              ...
              setHover({ row: rowIdx, x: e.clientX - r.left });
            }}
