 ...
 * Optionally pairs a sparkline below. The number counts up from 0 on mount/change
 * (honors prefers-reduced-motion; static for non-numeric values).
 */
...
export type KpiTileProps = {
  ...
  trend?: "up" | "down";
  /** Optional sparkline series shown below. */
  sparkline?: number[];
...
const VALUE_SIZE: Record<"md" | "lg" | "xl", string> = {
  md: "var(--text-36)",
  lg: "var(--text-48)",
  xl: "var(--text-60)",
};

type NumMeta = { value: number; decimals: number; comma: boolean; spaced: boolean; sign: boolean };

function resolveDir(delta: React.ReactNode, trend?: "up" | "down"): "up" | "down" {
...
function parseNum(raw: React.ReactNode): NumMeta | null {
...
function formatNum(value: number, meta: NumMeta): string {
  const neg = value < 0;
...
function useCountUp(raw: React.ReactNode, duration = 900): React.ReactNode {
  const meta = React.useMemo(() => parseNum(raw), [raw]);
  const [display, setDisplay] = React.useState<React.ReactNode>(raw);
  React.useEffect(() => {
    if (!meta) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- statisk visning for ikke-numeriske verdier
      setDisplay(raw);
      return undefined;
    }
    const reduce =
