  ...
  return (
    <StatsRamme mobile={mobile} waveId="marked-stats-min-progresjon">
      <Seksjon mobile={mobile}>
        ...
        <p style={{ fontFamily: TL.font.sans, fontSize: 15, color: TL.mute, marginTop: 18, maxWidth: 560, lineHeight: 1.6 }}>
          {harData
      ...
      {!harData && (
        <Seksjon mobile={mobile} style={{ paddingTop: 0 }}>
          <Kort tint pad="48px 32px" style={{ maxWidth: 560, margin: "0 auto", textAlign: "center" }}>
            <Icon name="trending-up" size={40} style={{ color: TL.fill, opacity: 0.5 }} />
            <div style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 20, color: TL.text, marginTop: 16 }}>Ingen SG-data ennå</div>
            <p style={{ fontFamily: TL.font.sans, fontSize: 14, color: TL.mute, lineHeight: 1.55, marginTop: 8 }}>
          ...
          <Kort pad={mobile ? "22px" : "32px"}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(3, 1fr)", gap: 24 }}>
              <div>
                ...
                <div style={{ fontFamily: TL.font.mono, fontSize: 44, fontWeight: 500, marginTop: 8, lineHeight: 1, color: TL.text }}>
                  {siste.sgDiffTotal !== null ? siste.sgDiffTotal.toFixed(1) : "—"}
                </div>
                <div style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, marginTop: 6 }}>per runde</div>
              </div>
              <div>
                <Caps>vs {siste.refPlayerName}</Caps>
                <div style={{ fontFamily: TL.font.mono, fontSize: 44, fontWeight: 500, marginTop: 8, lineHeight: 1, color: TL.fill }}>
                  {siste.sgDiffTotal !== null ? (siste.sgDiffTotal > 0 ? "+" : "") + siste.sgDiffTotal.toFixed(2) : "—"}
                </div>
                <div style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, marginTop: 6 }}>SG-differanse</div>
              </div>
              <div>
                <Caps>Tour-ekvivalent score</Caps>
                <div style={{ fontFamily: TL.font.sans, fontWeight: 700, fontSize: 26, marginTop: 8, color: TL.text }}>
                  {siste.estPgaTourScore !== null ? siste.estPgaTourScore.toFixed(1) : "—"}
                </div>
                <div style={{ fontFamily: TL.font.sans, fontSize: 13, color: TL.mute, marginTop: 6 }}>est. på PGA-bane</div>
              </div>
            </div>
            <div style={{ marginTop: 22 }}>
              <Link href="/stats/sg-sammenlign"><Knapp icon="arrow-right">Ny sammenligning</Knapp></Link>
      ...
      {trendSeries.length > 1 && (
        <Seksjon mobile={mobile}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(2, 1fr)", gap: AK.gap, marginTop: 20 }}>
            {KATEGORIER.map((kat) => {
              ...
              return (
                <Kort key={kat.key}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Caps>{kat.label}</Caps>
                    <span style={{ fontFamily: TL.font.mono, fontSize: 11, fontWeight: 700, color: bedre ? TL.ok : diff < 0 ? TL.danger : TL.mute, display: "inline-flex", alignItems: "center", gap: 4 }}>
                      <Icon name={ikon} size={12} />
                      {diff > 0 ? "+" : ""}{diff.toFixed(2)}
                    </span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 16 }}>
                    <span style={{ fontFamily: TL.font.mono, fontSize: 12, color: TL.mute }}>{fra.toFixed(1)}</span>
                    <span style={{ fontFamily: TL.font.mono, fontSize: 14, fontWeight: 500, color: bedre ? TL.ok : TL.text }}>→ {til.toFixed(1)}</span>
                  </div>
                  {values.length === 0 && <p style={{ fontFamily: TL.font.sans, fontSize: 12, color: TL.mute, marginTop: 8 }}>Ingen data ennå</p>}
                </Kort>
              );
            })}
          </div>
        ...
        <Seksjon mobile={mobile}>
          ...
          <Kort pad="0" style={{ marginTop: 24, overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", minWidth: 480, borderCollapse: "collapse", fontFamily: TL.font.mono, fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: `1px solid ${TL.hair}`, background: TL.dock }}>
                    <th style={{ padding: "12px 18px", textAlign: "left", fontWeight: 700, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: TL.mute }}>Dato</th>
                    <th style={{ padding: "12px 18px", textAlign: "left", fontWeight: 700, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: TL.mute }}>Referanse</th>
                    <th style={{ padding: "12px 18px", textAlign: "right", fontWeight: 700, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: TL.mute }}>SG-diff</th>
                    <th style={{ padding: "12px 18px", textAlign: "right", fontWeight: 700, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", color: TL.mute }}>Tour-score est.</th>
                  </tr>
                </thead>
                <tbody>
                  {sammenligninger.map((s, i) => (
                    <tr key={s.id} style={{ borderBottom: `1px solid ${TL.hair}`, opacity: i > 9 ? 0.55 : 1 }}>
                      <td style={{ padding: "12px 18px", color: TL.mute }}>{NB_DATO_KORT(s.createdAt)}</td>
                      <td style={{ padding: "12px 18px", color: TL.mute }}>{s.refPlayerName}</td>
                      <td style={{ padding: "12px 18px", textAlign: "right", color: TL.mute }}>
                        {s.sgDiffTotal !== null ? (s.sgDiffTotal > 0 ? "+" : "") + s.sgDiffTotal.toFixed(2) : "—"}
                      </td>
                      <td style={{ padding: "12px 18px", textAlign: "right", color: TL.fill, fontWeight: 700 }}>
      ...
      {siste?.kommentar && (
        <Seksjon mobile={mobile}>
          <div style={{ borderLeft: `3px solid ${TL.fill}`, paddingLeft: 24 }}>
            <div style={{ fontFamily: TL.font.sans, fontSize: mobile ? 17 : 20, fontWeight: 400, fontStyle: "italic", lineHeight: 1.6, color: TL.text }}>
              {siste.kommentar}
            </div>
      ...
      <Seksjon mobile={mobile}>
        <Kort tint pad={mobile ? "26px 22px" : "40px 44px"} style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1.2fr 1fr", gap: 32 }}>
          <div>
            <Caps color={TL.fill} style={{ marginBottom: 14 }}>Bli PlayerHQ PRO</Caps>
            <SeksT mobile={mobile} em="automatisk.">Spor alle SG-kategorier</SeksT>
            <p style={{ fontFamily: TL.font.sans, fontSize: 14, color: TL.mute, lineHeight: 1.65, margin: "14px 0 0", maxWidth: 460 }}>
          ...
          <div>
            <div style={{ fontFamily: TL.font.sans, fontSize: 13, fontWeight: 700, color: TL.text, marginBottom: 10 }}>Inkludert i PRO</div>
