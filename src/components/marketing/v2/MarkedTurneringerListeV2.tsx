...
import { PkSek, PkEyebrow, PkHero, PkIng, PkCta, PkKat, PkTom } from "./paper/PkPrimitives";
            ...
            {norskeDenneUka.map((e) => (
              <div key={e.id} className="pk-kort pk-kort-pad" style={{ flex: "none", minWidth: 220 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontFamily: "var(--p-disp)", fontWeight: 600, fontSize: 14.5, color: "var(--p-fg)" }}>{e.player.name}</span>
                </div>
                <div style={{ fontFamily: "var(--p-ui)", fontSize: 11.5, color: "var(--p-muted)", marginTop: 6, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {e.tournament.name}
                </div>
                {e.tournament.status === "IN_PROGRESS" && e.position !== null && (
                  <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 8 }}>
                    <span style={{ fontFamily: "var(--p-mono)", fontSize: 18, fontWeight: 700, color: "var(--p-accent-fg)" }}>{e.position}.</span>
                    {e.scoreToPar !== null && <span style={{ fontFamily: "var(--p-mono)", fontSize: 12, color: "var(--p-muted)" }}>{formaterToPar(e.scoreToPar)}</span>}
                  </div>
                )}
              </div>
            ))}
    ...
    <Link href={t.slug ? `/turneringer/${t.slug}` : "/turneringer"} className="pk-kort pk-kort-hover">
      <div className="pk-kort-body">
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontFamily: "var(--p-ui)", fontSize: 12, color: "var(--p-muted)" }}>
