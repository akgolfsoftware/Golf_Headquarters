/**
 * AK Golf-palett — domeneverdier som ikke tilhører noe designsystem.
 *
 * Opprettet 30.08.2026 da Paper ble fjernet fra hele plattformen (Anders).
 * Innholdet er FLYTTET ORDRETT fra det slettede src/lib/v2/tokens.ts — ingen
 * verdi er endret, så ingen piksler flyttet seg ved flyttingen.
 *
 * Hva som hører hjemme her: farger og mål som er AK Golfs egne fakta, ikke
 * stilvalg — pyramideaksene, fysiske tee-farger, tredjeparts merkefarger,
 * og delekort-gradienter som må være faste fordi kortet eksporteres som bilde
 * og derfor ikke kan lese CSS-variabler.
 *
 * Hva som IKKE hører hjemme her: flate-, tekst- og handlingsfarger. De bor i
 * src/lib/v2/train-lock.ts (TL) og er eneste designfasit.
 *
 * MERK om navnene i `farge`: de lyver, og gjorde det før flyttingen også.
 * `forestMerke*` og `limeMerke*` er navngitt etter en palett som ble erstattet
 * for lenge siden — verdiene er noe annet enn navnet sier. Les alltid VERDIEN.
 * Omdøping er en egen mekanisk jobb.
 */

export const AK = {
  // Kortskygge. Flyttet fra det slettede Paper-settet 30.08.2026, uendret verdi —
  // Train-lock har ikke noe skygge-token, og å legge til ett der krever
  // designbeslutning (CLAUDE.md invariant 2).
  skygge: "0 1px 0 rgba(20,20,19,0.03), 0 18px 40px rgba(20,20,19,0.05)",
  ...
  // lime → --tl-ok #63784A, med samme alfa som før. Navnene står igjen fordi
  ...
  farge: {
    ...
    // forestMerke var Presis-grønn → Paper-primær er blekk (--tl-fill).
    forestMerke: "var(--tl-fill)",
    ...
    // limeMerke var Presis-lime og bar «positiv/høydepunkt» → Paper --tl-ok.
    limeMerke: "var(--tl-ok)",
