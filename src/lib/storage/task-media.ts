...
export type UploadTaskMediaResult = { ok: true; url: string };
...
export async function uploadTaskMedia(
  ...
  if (fil.size > MAX_BYTES[kind]) {
    throw new Error(kind === "bilde" ? "Bilde er for stort (maks 5 MB)." : "Video er for stor (maks 50 MB).");
  }
  const ext = ALLOWED_TYPES[kind][fil.type];
  if (!ext) {
    throw new Error(kind === "bilde" ? "Bare JPG, PNG eller WEBP er tillatt." : "Bare MP4 eller MOV er tillatt.");
  }
