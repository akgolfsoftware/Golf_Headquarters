/**
 * Data-loader for AgencyOS Kalender — uke-visning, lese-modus (/admin/kalender).
 * Henter ekte booking-data for valgt uke fra Prisma og mapper til
 * WeekCalendarProps. Server-only — kalles fra page.tsx (RSC).
 *
 * Klassifisering av event-type (styrer venstre-strek-farge):
 *   - live    → økt pågår nå (now ∈ [startAt, endAt))  · cream-gull strek
 *   - group   → serviceType uten coach (felles/gruppe, f.eks. WANG) · forest strek
...
function mandagFor(d: Date): Date {
  const m = new Date(d);
  m.setHours(0, 0, 0, 0);
  // getDay(): 0=søn … 6=lør. Antall dager tilbake til mandag.
...
export async function loadWeekCalendar(ukeParam?: string): Promise<WeekCalendarProps> {
  const now = new Date();
