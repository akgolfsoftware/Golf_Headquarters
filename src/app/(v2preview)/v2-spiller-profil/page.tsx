/**
 * v2-forhåndsvisning — AgencyOS Spillerprofil 360° (retning C). Egen top-level
 ...
 *
 * Auth + datauthenting følger den ekte /admin/spillere/[id]-siden 1:1: samme
 ...
 * meldinger). Siden [id] ikke finnes i preview, hentes en EKTE eksempel-spiller
 * (første PLAYER) — ærlig tom-tilstand hvis stallen er tom.
 *
 * Server component.
 */
...
export default async function V2SpillerProfilPage() {
  const user = await requirePortalUser({ allow: ["ADMIN", "COACH"] });
  const now = new Date();

  // EKTE eksempel-spiller (første PLAYER). Ærlig tom-tilstand hvis ingen finnes.
      ...
      <V2Shell aktiv="spillere" nav={AGENCYOS_NAV} navn={user.name ?? "Coach"}>
        <Kort>
          <TomTilstand
            ...
            sub="Legg til en spiller i stallen for å se en spillerprofil her."
          />
        </Kort>
      </V2Shell>
    );
