/**
 * DEMO — Drill Challenge (steg 2 av 2)
 * Bygd fra wireframe live-states/e1-drill-challenge-steg2.html
 * URL: /drill-challenge-demo
 */
...
const SELECTED_FRIENDS: Friend[] = [
  ...
  { initials: "AK", name: "Anna Karlsen" },
  { initials: "MR", name: "Mads Rønning" },
...
export default function DrillChallengeDemo() {
  return (
      ...
      <div className="w-full max-w-[640px] rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          <div>
            ...
            <input
              type="email"
              placeholder="navn@klubben.no"
              defaultValue=""
              className="mt-2 w-full rounded-md border border-input bg-card px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          </div>

          {/* Toggle */}
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-border bg-card px-4 py-3.5">
            <span className="relative inline-block h-[22px] w-[36px] rounded-full bg-border">
              <span className="absolute left-0.5 top-0.5 h-[18px] w-[18px] rounded-full bg-background shadow-sm" />
            </span>
            <div className="flex-1">
              <div className="text-sm font-medium">Åpen for alle i klubben</div>
              <div className="mt-0.5 font-mono text-[11px] uppercase tracking-[0.04em] text-muted-foreground">
                GFGK-medlemmer kan bli med
              </div>
            </div>
          </label>
