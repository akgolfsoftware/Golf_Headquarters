...
function isoUke(d: Date): number {
  ...
  return (
    1 +
    Math.round(
      ((t.getTime() - forsteTorsdag.getTime()) / 86400000 -
        3 +
        ((forsteTorsdag.getUTCDay() + 6) % 7)) /
        7,
    )
  );
}
...
function aktivPeriode(perioder: CoachPeriode[]): CoachPeriode | null {
  ...
  return (
    perioder.find((p) => p.start <= iDag && iDag <= p.end) ??
    perioder[0] ??
    null
  );
}
...
export function CoachArsplan({ live = null }: { live?: WangLiveData | null }) {
  ...
  return (
    <div
      data-paper-slug="wang-coach-arsplan"
      style={{
        display: "flex",
        minHeight: "100vh",
        ...
        {perioder.map((p) => (
          ...
          >
            ...
            >
              ...
              >
                {dato(p.start).replace(/ \d{4}$/, "")} –{" "}
                {dato(p.end).replace(/ \d{4}$/, "")}
              </span>
            </div>
            <PyramideBar p={p} />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      >
        ...
        >
          ...
          >
            {dato(periode.start)} – {dato(periode.end)}
          </span>
        </div>
        <p
          style={{
            ...
            return (
              <div
                ...
                style={{
                  padding: 14,
                  borderRadius: 16,
                  background: aktiv ? "var(--tint-teal)" : "var(--neutral-50)",
                  border: aktiv
                    ? "1px solid var(--wang-mint)"
