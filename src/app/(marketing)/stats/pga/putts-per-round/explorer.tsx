...
/**
 * PuttsPerRoundExplorer — interaktiv sammenligning med slider.
 *
 * REVERSE: lavere er bedre.
 *
 * Bruker:
 *   1. Drar slider for antall putts per runde
 *   2. Ser percentile — andel spillere som tar FLERE putts enn deg
 *   3. Ser nærmeste proff på samme nivå
 *   4. Ser distribusjonsgraf
 *
 * Også: Topp 20-toppliste under slideren (sortert lavest → høyest).
 */
...
const TOPP_N = 20;
const BUCKET_SIZE = 0.5; // halvt putt pr bøtte

export function PuttsPerRoundExplorer({
  datapunkter,
}: {
  datapunkter: Datapunkt[];
}) {
  // Sortert lavest → høyest (lavere = bedre)
  const sortert = useMemo(
    () => [...datapunkter].sort((a, b) => a.verdi - b.verdi),
    [datapunkter],
  );

  const min = Math.floor(Math.min(...datapunkter.map((p) => p.verdi)) * 2) / 2;
  const max = Math.ceil(Math.max(...datapunkter.map((p) => p.verdi)) * 2) / 2;

  // Default: norsk amatør-snitt ~32 putts
  const [dittTall, setDittTall] = useState(32);

  // Percentile (reverse): andel spillere som tar FLERE putts enn deg
  const antallFlere = datapunkter.filter((p) => p.verdi > dittTall).length;
  const percentile = Math.round((antallFlere / datapunkter.length) * 100);
  ...
  const sliderMin = Math.max(0, min - 2);
  const sliderMax = max + 5;
        ...
        <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6 sm:py-16">
          <div className="text-center">
            <h2 className="font-display text-2xl font-semibold tracking-tight sm:text-3xl">
              Hvor mange putts tar{" "}
              <em className="font-normal italic text-primary">du</em>?
            </h2>
            <p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">
              Dra slideren til ditt gjennomsnittlige antall putts per runde,
              og se hvor du står sammenlignet med PGA Tour.
            </p>
          </div>
          ...
          <div className="mt-10">
            ...
            <div className="mt-6">
              <input
                ...
                aria-label="Dine putts per runde"
              />
              <div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground">
                <span>{sliderMin.toFixed(1)} (best)</span>
                <span>{sliderMax.toFixed(1)} (flest)</span>
              </div>
            </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      Du putter bedre enn {percentile}% av PGA-spillere.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-border bg-secondary/30 p-6">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {naermeste.spiller.verdi.toFixed(2)} putts (
                      {naermeste.diff < 0.05
                        ? "samme"
                        : `${naermeste.diff.toFixed(2)} ${
              ...
              {histogram.buckets.map((b) => {
                ...
                return (
                  <div
                    ...
                    title={`${b.start.toFixed(1)}–${(b.start + BUCKET_SIZE).toFixed(1)}: ${b.count} spillere`}
                  />
                );
              })}
            </div>
            <div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground">
              <span>{histogram.buckets[0]?.start.toFixed(1) ?? min}</span>
              <span>
                {(
          ...
          <div className="mb-6 flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Sesong 2026 · minst 20 runder spilt · færrest putts
              </p>
            </div>
          </div>
