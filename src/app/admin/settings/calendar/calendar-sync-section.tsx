/**
 * CalendarSyncSection — gjenbrukbar Google Calendar-kobling + kalendervelger.
 *
 * Brukes både på /admin/settings/calendar og direkte på /admin/availability
 * (Tilgjengelighet) slik at en coach setter arbeidstider OG velger hvilke
 * kalendere som blokkerer booking på ÉN skjerm. Laster connection + abonnementer
 * for `userId` og rendrer status-kort + SubscriptionsForm (syncPull/syncPush).
 */
...
export async function CalendarSyncSection({
  userId,
  ok,
  error,
}: {
        ...
        <>
          <section className="rounded-2xl border border-border bg-card p-6">
            ...
            <dl className="mt-6 grid gap-4 sm:grid-cols-3 text-sm">
              ...
              />
              <Rad label="Antall kalendere" value={String(rader.length)} />
              {conn.lastError && (
