/**
 * AgencyOS — Stats-oversikt / admin-dashboard (`/admin/stats/overview`), v2.
 * Port av `(legacy)/stats/overview/page.tsx` (2026-07-14, AgencyOS
 * Bølge 3.19) — datahenting er 100% uendret (inkl. den forhåndseksisterende
 * `hentSisteCommits()`-feilen med hardkodet lokal filsti, se merknad).
 * `(legacy)/stats/overview/actions.ts` bor fortsatt der — delt
 * `sjekkDbHelse`-server-action, uendret.
 */
...
import { AdminStatsOversiktV2, type SyncRadV2, type CommitV2 } from "@/components/admin/v2/AdminStatsOversiktV2";
import { sjekkDbHelse } from "@/app/admin/(legacy)/stats/overview/actions";
...
// MERK: hardkodet lokal sti (Anders' maskin) — finnes aldri i Vercel/sandkasse.
// Fanges av try/catch → tom liste → seksjonen skjules. Forhåndseksisterende
// bug i legacy, bevart uendret her (meldt i MASTER-SKJERMPLAN.md).
function hentSisteCommits(): CommitV2[] {
  try {
...
async function hentAdminOverview() {
  const [
    ...
    prisma.pgaPlayerSeason.findFirst({ orderBy: { lastUpdated: "desc" }, select: { lastUpdated: true, playerName: true } }),
    prisma.tournament.findFirst({ where: { sourceOrigin: "DATAGOLF", lastSyncAt: { not: null } }, orderBy: { lastSyncAt: "desc" }, select: { lastSyncAt: true, name: true } }),
...
export default async function AdminStatsOverviewPage() {
  ...
  return (
    <V2Shell nav={AGENCYOS_NAV} navn={user.name} avatarUrl={user.avatarUrl}>
      <AdminStatsOversiktV2
        data={{ ...data, klokkeslettTekst: now.toLocaleTimeString("nb-NO", { hour: "2-digit", minute: "2-digit" }) }}
        sjekkDbHelse={sjekkDbHelse}
      />
    </V2Shell>
  );
}
