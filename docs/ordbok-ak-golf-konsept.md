# AK Golf — staving av appord (del B)

**Oppdatert 15.09.2026.** Del A (fagbegreper, koder, treningsmodell) er slettet — fasiten er
[ordbok-master-trening.md](ordbok-master-trening.md). Denne fila er kun **del B: hvordan
vanlige appord staves i skjermtekst**. Ved konflikt om betydning, koder eller enheter vinner
masteren. Historiske rader her (L-faser, CS, M/PR, gamle avstandsbånd) er ikke gjeldende.
...
## B13. Knapp-tekst & CTA-er

→ Flyttet til `docs/ordbok-master-trening.md` §5 (tekster + stiler + app-idiom).
...
→ Flyttet til `docs/ordbok-master-trening.md` §6.

## B20. Hero-titler

→ Flyttet til `docs/ordbok-master-trening.md` §7.

## B21. Personas (demo-data)

→ Flyttet til `docs/ordbok-master-trening.md` §8. Demo-kanon: Øyvind Rohjan (spiller) + Anders Kristiansen (coach); Markus Røinås Pedersen er EKTE coach, aldri demo-spiller.

## B22. Klubber & lokasjoner (demo-data)

→ Flyttet til `docs/ordbok-master-trening.md` §9.
...
1. **Endre i canon først** (MasterBrain) hvis det gjelder konsept-verdier — deretter denne fila → `docs/ordbok-master-trening.md` (visning) → `docs/ordbok.json` (regenerer med `npx tsx scripts/ordbok-json.ts`) → kode.
...
**Sist oppdatert:** 2026-07-03 — renskrevet fra 1629 til denne versjonen uten terminologitap (SLETTET/DEDUPE-lister i git-historikken). Synket mot MasterBrain CANON v3.5. Del B var opprinnelig egen fil (`ordliste-ak-golf.md`). Demo-kanon: Øyvind Rohjan.
