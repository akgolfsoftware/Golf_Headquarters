...
Modal med alle inputs i seksjonert layout:

**Seksjon 1: Beskrivelse**
- Tittel input + beskrivelse textarea

**Seksjon 2: Kategorisering**
...
**Seksjon 3: Trenings-modalitet (MORAD-skala)**
- L-fase / CS-nivå / M / PR — alle som segmented controls

**Seksjon 4: Bilde/video** (slot-design med upload-placeholder)

**Seksjon 5: Rep-mål** (3 separate tall: dry / lav / full)

**Seksjon 6: TM-mål per oppgave (NY — viktig!)**
Coach + spiller setter konkrete TrackMan-mål som måler om den tekniske
endringen virker. Hovedmål er **å redusere spredning**, ikke heve
absoluttverdier.

Layout:
- Toggle øverst: "☑ Mål spredning som primær (anbefalt)"
- Tabell med 5-6 målinger, hver med:
  - Måling-navn (eks "Dispersion", "Spin Axis avvik")
  - Baseline-verdi (auto-fylt fra siste 30d TM, kan overstyres)
  - Mål-verdi (input)
  - Status-pill: ● Primær / ○ Sekundær / ▷ Kausal

Eksempel-rad (for "V. håndledd flat" på 7-jern):
| Måling           | Baseline       | Mål             | Type        |
|------------------|----------------|-----------------|-------------|
| Dispersion       | 12.4 m std     | < 8 m std       | ● Primær    |
| Spin Axis avvik  | ±4.2°          | ±2°             | ● Primær    |
| Smash std        | 0.034          | < 0.020         | ● Primær    |
| Smash mean       | 1.39           | ≥ 1.42          | ○ Sekundær  |
| Spin Rate        | 7820 rpm       | 7000±400        | ○ Sekundær  |
| Dynamic Loft     | +2.3° cupping  | ±0.5°           | ▷ Kausal    |

Footer i seksjonen:
- "+ Legg til måling"-knapp
- Hjelpetekst: "Baseline hentes automatisk fra siste 30 dagers TM-data
  på {kølle} ({antall} slag). Justér om du vil sette mål utenfor det
  databaserte utgangspunktet."
- 2 felter: "Forventet tid: [6-8 uker ▾]" + "Neste evaluering: [29. juni ▾]"

4 mål-typer (forskjellige farger):
- **Primær** (rød/forest dot): Spredningsmål — dispersion, std, axis avvik
- **Sekundær** (gul/amber dot): Verdimål — smash mean, ball speed, carry
- **Kausal** (lilla dot): Tekniske parameter-mål — dynamic loft, attack angle
- **Hit-rate** (lime dot): Pass/fail-protokoll — 8/10 innenfor ±2° face angle

**Seksjon 6.5: Hit-rate-mål (NY — drill-protokoller!)**
Mens Mekanisme 6 (over) måler statistiske mål (snitt + spredning), måler
hit-rate **kontroll i øyeblikket** — som biathlon eller dart. En oppgave
kan ha både spredningsmål OG hit-rate-mål samtidig.

Layout:
- Toggle øverst: "☑ Bruk hit-rate-mål for denne oppgaven"
- En eller flere hit-rate-rader, hver med:
  - Måling-dropdown (Face Angle / Carry / Smash / Spin Rate / Club Path)
  - Kølle-dropdown
  - Korridor-input: "fra [-2.0]° til [+2.0]°"
  - Protokoll-dropdown:
    - Rullende vindu — siste N slag (default)
    - Beste i én økt — best-of-N
    - Streak — N treff på rad
    - Økt-gate — én økt må passere før neste fase
  - Krav-input: "[8] av [10] slag innenfor korridor"
  - Nå-status-rad: "7/10 i siste vindu · Best i økt: 8/10 · Streak: 4"
  - lime celebration når mål nås

Eksempel-rad (for "Stabilt face ved impact" på 7-jern):
| Måling      | Kølle  | Korridor    | Protokoll      | Krav   | Status        |
|-------------|--------|-------------|----------------|--------|---------------|
| Face Angle  | 7-jern | ±2°         | Rullende vindu | 8/10   | 7/10 ● lime   |
| Face Angle  | 7-jern | ±1°         | Rullende vindu | 6/10   | 5/10 ○ amber  |
| Face Angle  | 7-jern | ±2°         | Best i én økt  | 10/10  | 8/10 best     |

Hjelpetekst: "Hit-rate måler hvor ofte du treffer en korridor. 8/10 face
angle innenfor ±2° betyr kontroll. Sammen med spredningsmålet over måles
både kontroll-i-øyeblikket (hit-rate) og langtidsspredning (Mekanisme 6)."

**Seksjon 7: Linkede drills fra øvelsesbank** (chips, multi-select)

**Seksjon 8: Forhåndsvisning** (hvordan oppgaven ser ut i plan-listen)

**Knapp-rad (modal footer):**
- "Slett oppgave" (rød ghost, kun ved redigering)
- "Avbryt" + "Lagre oppgave" (primary)

VIKTIG OM TM-MÅL-SEKSJONEN:
- Bruk subtile fargekoder for å skille primær/sekundær/kausal
- Baseline-felt skal være read-only by default med "Overstyr"-link ved siden
- Hvis spilleren har <50 slag på køllen siste 30d, vis warning: "Trenger
  mer TM-data for å sette baseline. Foreslår å kjøre en baseline-økt først."
- Mål-input støtter både enkeltverdi (eks "< 8") og range (eks "6500-7500")
