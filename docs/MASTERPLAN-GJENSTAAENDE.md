...
| 15.11 | ~~**Stall-lista slankes**~~ **LEVERT, IKKE MERGET (31.08.2026).** Raden på `/admin/spillere` viser nå de fire tingene: navn · «Neste: i dag 13.00»/«Ingen økt planlagt» · «økt i går»/«innlogget 8 dg siden» · én prikk (fylt = trenger deg, åpen = følg med, ingen = på planen — samme kilde som bolkene). SG/hcp fjernet fra raden (fantes alt i detaljpanelet + `/admin/spillere/[id]`); **pakke var en død prop og vises nå i detaljpanelet** sammen med skylder/etterlevelse. Loaderen fikk «neste økt» (vinduet utvidet 14 d frem — den så bare bakover) og «siste gjennomførte økt»; etikettene er TruthLayer-ærlige om kilden (økt vs. innlogging, aldri «logget» uten kilde) og Oslo-korrekte (låst av `src/lib/admin/stall-rad.test.ts`, inkl. midnattstesten). Bolker/søk/filter beholdt (funksjonalitet fjernes ikke; canvasens flate liste uten søk er avvik, dokumentert). **Gjenstår:** skjermbilde-gate før merge · «Oppfølging»-fanen fra `/admin/queue` (canvasens «slås sammen fra»-kort, beslutning 6.6) er IKKE med — signalberegningen ligger inline i `queue/page.tsx` og må ekstraheres først; egen økt | — | Beslutning 6.5 |
