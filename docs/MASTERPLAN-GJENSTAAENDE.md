...
| # | Oppgave | Detalj |
|---|---|---|
| 17.1 | **Poster, ikke chat** | Trener poster til gruppe og til enkeltspiller, med video, bilder, lenker og vedlegg (flybilletter, hotellreservasjoner). **Ingen fri chat.** 1:1-poster til mindreårige skal være sporbare og synlige for forelder (idrettens åpenhetsprinsipp) — det er et krav til datamodellen, ikke en UI-detalj. **TN-10 Post til enkeltspiller LEVERT 31.08.2026** (`templates/tn-post-enkeltspiller/`) — forelder/foresatt vises som synlig mottaker ved siden av mindreårig spiller, kun spilleren når myndig. **TN-09 Grupper med poster LEVERT 01.09.2026** (`templates/tn-gruppeposter/`) — samme postmekanikk som TN-10, mottaker er gruppen; lesekvittering som brøk med «Se hvem» |
| 17.2 | **Dokumenter med lesekvittering** | Fildeling per gruppe, «12 av 14 har åpnet uttakskriteriene», og «sist oppdatert»-merking. **TN-11 Dokumentdeling LEVERT 01.09.2026** (`templates/tn-dokumentdeling/`) — brøk i raden, navneliste med hvem som mangler i panelet ved siden av |
| 17.2b | **Samtykke og deling per organisasjon (spillersiden)** | To brytere per organisasjon (tester/resultater, komplett profil), aldri ti — jf. §FORRETNINGSMODELL: SPILLERLISENSER. **TN-12 Samtykke LEVERT 01.09.2026** (`templates/tn-samtykke/`) — tegnet i foreldrevisning for mindreårig spiller. Hører hjemme i PlayerHQ/Forelder, ikke under `/team-norway/*` — se designnotatet i filen |
