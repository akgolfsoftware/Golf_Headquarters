...
| OW-3 | Én økt-modell: `WorkbenchSession` og `TrainingPlanSession` slås sammen så coach og spiller redigerer samme økt | `prisma/schema.prisma` (6165, 1323), `src/lib/workbench/wb-actions.ts`, `src/app/portal/planlegge/workbench/actions.ts` | Én tabell for økt; `BUSINESS-RULES.md` «Workbench er delt kjerne» stemmer; eksisterende Workbench-tester grønne; datamigrering godkjent særskilt |
