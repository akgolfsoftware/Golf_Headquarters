...
| OW-2 | **Delvis gjort 15.–16.09.2026.** Fem områdefamilier (Bunker under Nærspill), «Utslag» som eneste OTT-navn i appens egen visning (17 steder) og «Ikke fullført» for DNF i foreldreflaten er rettet. Periodelistene er slått sammen: `LPhase` og `PeriodeType` har nå samme åtte verdier og samme skrivemåte (SPESIAL, ikke SPESIALISERING) — lagt til additivt (`ALTER TYPE ... ADD VALUE`/`RENAME VALUE`, 0 rader berørt, verifisert mot basen først), og den gamle kalenderens lossy EVALUERING→TURNERING/FERIE→GRUNN-gjetning (`periode-helpers.ts`) er fjernet. `ordbok-json` og en grep-vakt mot utgåtte ord er lagt i `npm run verify:static`. **Rest:** `src/lib/taxonomy.ts` og `src/components/workbench-hybrid/taxonomy.ts` (den gamle 17-områdelisten) er IKKE redusert — de står som kjente, unntatte brofiler (se `scripts/ordbok-json.ts` UNNTAK-lista), ikke rettet mot masteren. GFGK_BREDDE/JENTER → BASIS/UTVIKLING er en datamigrering og krever egen godkjenning (Anders plasserer spillere selv) — ikke gjort. | `src/lib/domain/ak-formel-v2.ts`, `prisma/schema.prisma`, `scripts/ordbok-json.ts`, `scripts/fiks-periodelister-2026-09-16.ts`, `package.json` | Verify feiler hvis dokument og kode spriker eller et utgått ord brukes utenfor migreringsbroene. Full gjenstående rest: taxonomy.ts-reduksjonen og GFGK-migreringen |
