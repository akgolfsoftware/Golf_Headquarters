...
Aktiv arbeidsdeling 13.09.2026: Claude Design eier Design System v0.1. GitHub hadde etter PR #882 ingen åpne pull requests eller issues ved denne avstemmingen. Ikke bygg fra kandidat v0.4.17/18 før en pakke er valgt for det aktuelle omfanget (port: CD-1/CD-2).

| Prioritet / ID | Konkret neste leveranse | Inngang | Ferdig når |
|---|---|---|---|
| 1 · D0 | Fullfør og velg én Claude Design-pakke for et tydelig registrert omfang. **Port (15.09):** velges ikke før Workbench v0.7 og kalenderfamilien er sett av Anders på 390 px (CD-1, CD-2) | Kandidat v0.4.17/18 (prosjekt `047cfd41`) og designregisteret; avklar P11, O11 og O12 | Pakken har versjon, hash, komplett avtalt dekning og `selectedForBuilding: true`; Anders har valgt den |
| 1a · CD-1 | Masteren inn i Claude Design: `docs/ordbok-master-trening.md` lastes opp i prosjektet, `AGENCYOS-AUTORITET.md` §1 peker på den i stedet for 08.09-ordboken | Claude Design `047cfd41`, `uploads/` | Autoritetsfilen oppgir masteren som eneste ordkilde; ingen fil i `leveranse/` siterer 08.09-ordboken |
| 1b · CD-2 | Workbench v0.7-prompt: fasilitet først med mål, reps per motorikk-steg, gruppeuke med kopi til spillere, delt økt med ansvarlig trener, teknisk plan ↔ P-oppgaver ↔ TrackMan-mål. **Prompt og pakke levert 15.09** ([prompt](planer/design/2026-09-15-claude-design-workbench-v0-7-prompt.md), ZIP i Drive `claude-cowork/akgolf-hq/ak-golf-hq-workbench-v0-7-claude-design-2026-09-15.zip`); Anders slipper `kilder/` inn og sender prompten | `docs/planer/design/2026-09-15-claude-design-workbench-v0-7-prompt.md` | v0.7 finnes i prosjektet, klikkprøvd 390/834/1440, WB-05–WB-10 + kalender dag/uke/måned/agenda i én fil; Anders har sett den på 390 |
| 1c · CD-3 | Ved valg: speil `leveranse/` inn som `designsystem/<retning>/`, pek `check-fasit-sitering.mjs` og `check-fasitdekning-baseline.mjs` om fra `designsystem/train-lock`, merk Train-lock historikk i `designsystem/README.md` | `scripts/`, `designsystem/` | `npm run verify` grønn mot ny mappe; ingen kodefil siterer Train-lock som fasit |
...
| 3 · R-A–R-E | **Fullført innenfor trygt omfang 14.09.** Abonnement/tilgangsnivå i main via PR #886. Caddie-tilgang (API-et, ingen side finnes ennå), TrackMan CSV+HTML og privat lokal lagring (R-C) bestått, gren `codex/etter-merge-2026-09-14`. **Bevisst udekket:** Caddie sitt AI-svar og TrackMan foto-kilde — begge krever `ANTHROPIC_API_KEY`, som ikke skal inn i et sky-testmiljø | Eksisterende sikkerhetsregler og PR #877 | Tillatt rolle virker, uvedkommende avvises, feiltilstander er ærlige og ingen persondata sendes ukontrollert |
...
### Ordbok og Workbench-planlegging — rekkefølge låst 15.09.2026

Kilde: `.claude/rules/beslutninger.md` §ORDBOK-MASTER FØRST, SÅ ÉN ØKT-MODELL, SÅ GRUPPEPLANLEGGING. Grunnlag: [analysen](beslutningsgrunnlag/ordbok-og-workbench-analyse-2026-09-15.md). Radene gjøres i rekkefølge; OW-P kjøres parallelt. Hver skjerm følger tegn-før-bygg og skjermbilde-gaten.

| ID | Konkret leveranse | Hvor i koden | Ferdig når |
|---|---|---|---|
| OW-1 | **Gjort 15.09.2026 kveld.** Anders svarte på alle 13 avklaringer i kap. 19 (tabellen der er fasit); svarene er lagt inn i kapitlene. Gamle ordbøker slettet tidligere samme dag. Masteren har status GJELDENDE KILDE | `docs/ordbok-master-trening.md` | Oppfylt. Rest: konsekvensene er ført inn i OW-2, OW-4, OW-6 under |
| OW-2 | Vokabularet i koden: `src/lib/taxonomy.ts` og `src/components/workbench-hybrid/taxonomy.ts` reduseres til det masteren har; `docs/ordbok.json` genereres fra kodens lister; `ordbok-json` og en grep-vakt mot utgåtte ord (L_KROPP, CS40, M0, PR1, INNSPILL_0_50, PUTT_10_15) legges i `npm run verify`. **Fra kap. 19 (15.09):** fem områdefamilier (Bunker under Nærspill), én periodeliste med alle åtte, «Utslag» som eneste OTT-navn, «Ikke fullført» for DNF i foreldreflaten, GFGK_BREDDE/JENTER → BASIS/UTVIKLING (datamigrering godkjennes særskilt, Anders plasserer spillere) | `src/lib/domain/ak-formel-v2.ts`, `scripts/ordbok-json.ts`, `package.json` | Verify feiler hvis dokument og kode spriker eller et utgått ord brukes utenfor migreringsbroene |
| OW-3 | Én økt-modell: `WorkbenchSession` overlever; `TrainingPlanSession`-økter migreres inn (beslutning 15.09 §WORKBENCH-MOTOREN). Avgjør samtidig hva som skjer med `TrainingSessionV2` (gjennomføring/live) | `prisma/schema.prisma` (6165, 1323, 3642), `src/lib/workbench/wb-actions.ts`, `src/app/portal/planlegge/workbench/actions.ts`, `src/lib/workbench/session-actions.ts` | Én tabell for planlagt økt; `BUSINESS-RULES.md` «Workbench er delt kjerne» stemmer; eksisterende Workbench-tester grønne; datamigrering godkjent særskilt |
| OW-3b | Coach bruker `WorkbenchV2`: stall-velger og gruppe-modus legges oppå spillerens komponent; serie, «lagre som mal» og inspektør flyttes fra `WorkbenchUke` inn i motoren; `/admin/workbench/[playerId]` rendrer `WorkbenchV2` | `src/components/portal/v2/WorkbenchV2.tsx`, `src/components/workbench/WorkbenchUke.tsx`, `src/app/admin/workbench/[playerId]/page.tsx` | Coach og spiller ser samme uke i samme komponent; `WorkbenchUke` slettet; skjermbilder 390/1280 lys og mørk mot WB-01/WB-02 |
| OW-4 | Gruppeuke i Workbench: coach lager én gruppeøkt, den kopieres til hver spiller med `sourceGroupSessionId`, publisering og «ikke delta». **Fra kap. 19 (15.09):** coach-eide treningssteder; gruppeøkten beholder coachens sted hos spilleren | `src/lib/domain/gruppesynk.ts`, `src/components/workbench/`, fasit WB-08/WB-09/WB-10 | Innlogget reise: gruppeøkt → vises hos alle medlemmer → én spiller skjuler → coach ser det; skjermbilder 390/1280 lys og mørk |
| OW-5a | Ny uke starter aldri tom: «kopier forrige uke» er default når en uke uten økter åpnes, for coach og spiller (beslutning 15.09 §WORKBENCH-MOTOREN). Én bekreftelse, angre mulig | `src/lib/workbench/duplicate-week.ts`, `coachDuplicateWeek` (`session-actions.ts:217`), `duplicateWorkbenchWeek` (`portal/planlegge/workbench/actions.ts:673`) | Tom uke viser forrige ukes økter som utkast med ett trykk; ingen økt publiseres automatisk |
| OW-5 | Årsplanen fyller uka: forslag bygges av `weeklySessionBudget`, `GroupSchedule`, `SchoolScheduleEntry` og `TournamentEntry`; coach retter | `src/lib/workbench/perioder.ts`, `src/lib/workbench/load-workbench.ts` | Tom uke i en periode får et ferdig forslag; coach kan endre alt; ingen regel håndheves |
| OW-6 | Uke-maler per periode og kategori: `PlanTemplate` får periode og `NgfKategori`, velges automatisk i OW-5. **Fra kap. 19 (15.09):** uketype (master 4.4) blir valgfritt felt på uke i årsplanen; malen kan knyttes til uketype. Nytt felt godkjennes særskilt | `prisma/schema.prisma` (PlanTemplate), `src/lib/workbench/apply-template-actions.ts` | «Grunnperiode-uke, kategori E» kan lagres én gang og brukes i alle grupper |
| OW-7 | Styrkeøkt (sett, reps, hvile, % av 1RM, RIR) og testdag med bulk-føring planlegges og føres inne i Workbench, ikke på egne flater | `FysOvelseRad`, `TestAssignment`, `src/components/workbench/` | Coach planlegger styrkeøkt for gruppe fra Workbench; ti spillere føres etter tur på samme protokoll |
| OW-8 | Delt økt: blokker under økta med start/slutt og ansvarlig coach per blokk. Tegnes i Claude Design før bygging | Ny modell (delblokk), `src/components/workbench/SessionInspector.tsx` | Canvas godkjent av Anders; deretter bygget og sett |
| OW-9 | Coach-årsplanen (WB-06) kan redigere perioder; gruppe får 12-mnd-oversikt | `src/components/workbench/YearPeriodePanel.tsx`, `YearGrid.tsx` | Periode kan opprettes og endres fra årsvisningen for spiller og gruppe |
| OW-P | Parallelt: (8) `canon-methodology.json` og RAG-korpuset ryddes for L-faser/CS/prosentkrav så Caddie leser masteren; (11) `/beslutning` samme dag; (12) tegn før bygg; (13) `/design-login` og last masteren inn i Train-lock-prosjektet | `src/lib/masterbrain/`, Claude Design `a5152cf9` | Caddie foreslår ikke regler som er avskaffet; masteren ligger som referanse i Claude Design |
...
| Plan/Live | Frekvens, FYS-gjenåpning og serverregler for ny/rediger/flytt er prøvd. Innlogget ny/rediger/flytt og Caddie i live gjenstår | [P02–P05](design-audit/plan-live-p02-p05-2026-09-12.md) · [ny/rediger/flytt](design-audit/plan-ny-rediger-flytt-2026-09-13.md). Modellene slås sammen i OW-3 (beslutning 15.09.2026) |
| Workbench · O02 | Plan-publisering avviser uvedkommende og bruker norsk mandag. **14.09: gruppeutrulling er låst i test — samme gruppe er stille idempotent, krysskilde hoppes og rapporteres, og spillerens egne økter blir liggende urørt.** Rest: innlogget brukerreise for gruppeplan. Videre gruppeplanlegging: OW-4 | [Kontroll](design-audit/workbench-o02-publisering-2026-09-13.md) · [KODE-A](design-audit/kode-a-flake-og-o02-2026-09-14.md). PR #866 |
...
| WANG/GFGK | Årsplan, juniorgrupper, rapporter og foresatte. Testdager og styrkeprogram er flyttet til OW-7 | Virkelige rollegrenser og avklarte fagregler; P08/O03 i funksjonsregisteret |
...
| Kodekontroll · R-I | PR #848, #856, #866, #869 og #870 i main. Helseskriving i tidligere leveranse. **14.09: 31 nye avvisningstester for `admin/plans/[planId]/actions.ts` (16 handlinger) og `admin/grupper/[id]/actions.ts` (5 handlinger) — begge filene var allerede riktig eierskaps-sjekket i koden, men manglet søskentest.** **14.09 kveld: `(legacy)/approvals/actions.ts` (5 handlinger, 5 tester) — den mest følsomme av de udekkede, siden den endrer spillerens plan og varsler spilleren.** 48 admin-mutasjonsfiler mangler fortsatt søskentest; størst uten er `tournaments/actions.ts` (12 handlinger), `(legacy)/plan-templates/actions.ts` (10), `(legacy)/anlegg/location-actions.ts` (6) og `(legacy)/bookinger/actions.ts` (5) | [R-I](design-audit/handlingstilgang-r-i-2026-09-12.md) · [bred](design-audit/handlingstilgang-bred-2026-09-12.md) · [O02](design-audit/workbench-o02-publisering-2026-09-13.md) · [utstyrsbag](design-audit/handlingstilgang-utstyrsbag-2026-09-13.md) · [admin-spiller](design-audit/handlingstilgang-admin-spiller-2026-09-13.md) · [helse](design-audit/handlingstilgang-helse-2026-09-13.md) · [plans+grupper](design-audit/handlingstilgang-plans-grupper-2026-09-14.md) |
