...
|---|---|---|---|
| OW-1 | Anders retter og godkjenner `docs/ordbok-master-trening.md` (svar på kap. 19). Deretter slettes/krympes de gamle ordbøkene etter analysen §2, med lenkeretting | `docs/`, `.claude/rules/beslutninger.md`, `scripts/ordbok-json.ts`, `src/lib/masterbrain/rag-corpus/` | Masteren har status GJELDENDE; ingen annen fil kaller seg ordforråds-fasit; `check-doc-lenker` grønn |
| OW-2 | Vokabularet i koden: `src/lib/taxonomy.ts` og `src/components/workbench-hybrid/taxonomy.ts` reduseres til det masteren har; `docs/ordbok.json` genereres fra kodens lister; `ordbok-json` og en grep-vakt mot utgåtte ord (L_KROPP, CS40, M0, PR1, INNSPILL_0_50, PUTT_10_15) legges i `npm run verify` | `src/lib/domain/ak-formel-v2.ts`, `scripts/ordbok-json.ts`, `package.json` | Verify feiler hvis dokument og kode spriker eller et utgått ord brukes utenfor migreringsbroene |
...
| OW-6 | Uke-maler per periode og kategori: `PlanTemplate` får periode og `NgfKategori`, velges automatisk i OW-5 | `prisma/schema.prisma` (PlanTemplate), `src/lib/workbench/apply-template-actions.ts` | «Grunnperiode-uke, kategori E» kan lagres én gang og brukes i alle grupper |
