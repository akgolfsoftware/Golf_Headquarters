...
| 0.2 | **Rotér `SCREENTEST_PASSWORD`** — kompromittert 03.08; blokkerer ALLE sign-off-gallerier | **Anders** | `scripts/roter-screentest-passord.ts` |
...
| # | Oppgave | Blokkert av |
|---|---|---|
| 2.1 | **Sign-off-fabrikken C1–C7:** galleri (app+fasit, m390+d1280, lys+mørk) for de 44 `[~]`-radene, bølge for bølge — Anders setter `[x]` | 0.2 (passordet) |
...
| # | Oppgave | Kilde |
|---|---|---|
| 3.1 | **TalentHQ leser `testNivaaer`:** syncen (T4) skriver feltet, ingen skjerm leser det — koble talent-skjermene på | Kartlegging 17.08 |
| 3.2 | **Talent-gate-asymmetrien:** `/portal/talent`-huben er ugated men de fire underskjermene 404-er når `NEXT_PUBLIC_FEATURE_TALENT=false` — gate huben likt, eller slå på flagget | Kartlegging 17.08 |
...
| 3.6 | **Pyramidefordeling — spilleren setter egen:** plan ligger (`docs/plan-egen-pyramidefordeling-2026-08-02.md`); den kritiske feilen står fortsatt (`plan-action-executor.ts` overskriver `targetAllocation` ved periodebytte) | Verifisert 17.08 |
| 3.7 | **Live-økt-rest:** offline-kø for drill-reps + DB-persist (i dag sessionStorage) | STATUS-NÅ (eldre) |
| 3.8 | **Enhetstest for `protocol.ts`** — eneste utestede ledd i test-kjeden (Zod-normalisering av begge protokollvarianter) | Kartlegging 17.08 |
...
| # | Oppgave |
|---|---|
| 4.1 | **Turnerings-dedupe:** retningsfeilen fra 18.07 (NGF-stub vinner over fersk robot-rad) er IKKE kodefikset — fiks `scripts/dedupe-tournament-data.ts` til å foretrekke fersk scraper-origin, og verifiser `sourceOrigin in (OLYO,OSTLANDS) + mergedIntoId != null ≈ 0` |
