...
| OW-3 | **Fase 1–2 gjort 16.09.2026 (Anders: «gjør din anbefaling på disse 5 oppgavene»).** `WorkbenchSession` utvidet additivt (11 nye felt, String ikke enum) og alle 12 `training_plan_sessions`-rader migrert inn (idempotent, verifisert). `TrainingSessionV2` består som eget live-lag, re-pekes til `WorkbenchSession` i fase 4 (ikke gjort ennå). **Rest: fase 3 (rute om ny-skriving fra `/portal/planlegge/workbench` til `wb-actions.ts`), fase 4 (`v2-sync.ts`), fase 5 (~35 lesefiler i fem grupper A–E), fase 6 (opprydding)** | [Migreringsplan](planer/ow-3-en-oekt-modell-2026-09-16.md) med beslutningslogg; `prisma/schema.prisma` (6165, 1323, 3642), `src/lib/workbench/wb-actions.ts`, `src/app/portal/planlegge/workbench/actions.ts`, `src/lib/workbench/session-actions.ts` | Én tabell for planlagt økt; `BUSINESS-RULES.md` «Workbench er delt kjerne» stemmer; eksisterende Workbench-tester grønne; datamigrering godkjent særskilt |
