...
| 18.24 | **Skill for merkevarearbeid — GJORT 01.09.2026** | `.claude/skills/ak-merkevare/` — finjusterer MERKET (marked, materiell, presentasjon, sosialt, e-post, trykk, foto, tekst). Åtte sjekkpunkter i rekkefølge, de fire reglene som avgjør mest, forbudt-liste, filkart og WCAG-formelen så kontrasttall regnes og ikke anslås. **Grensen er innebygd:** skillen avviser produktskjermer og peker på `ak-designekspert`. Verifisert at hver fargeverdi i skillen finnes i `tokens/farge.css`. **Merk:** `ak-designekspert` er skrevet for Train-lock og vil avvise verkstedet — den skal ikke brukes på merkematerialet |
