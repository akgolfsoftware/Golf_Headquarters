...
| Ø9 | tir 15.09 | F2 AgencyOS | Coachens Workbench uke (D2 svart 02.09: WB-serien for struktur — ublokkert) | `/admin/workbench/[playerId]` | `WB-01 Uke minimum.dc.html` (struktur) · `A-01 Mac Uke Pro.dc.html` (Mac-piksel) | Agent |
| Ø10 | ~~ons 16.09~~ **delvis 02.09** | F2 AgencyOS | Publiser-bekreftelse + proveniens-banner. **Gjort:** gap dokumentert i `PublishConfirmDialog.tsx` — dagens dialog publiserer alle utkast samlet, WB-03s per-økt-checkbox/«Velg alle»/partial-publish er ikke bygget (ekte funksjonsutvidelse). A-09s Caddie-proveniensbanner krever en AI-forslagsfunksjon i coachens Workbench som ikke finnes ennå — parkert til beslutning (samme klasse som D7/Jarvis), ikke bygget som liksom-banner uten ekte agent bak (TruthLayer). **Gjenstår:** selve WB-03-funksjonen (checkbox-utvelgelse) når Anders prioriterer den | samme | `WB-03 Publish confirm 3 skall.dc.html` · `A-01d Publish confirm.dc.html` · `A-09 Mac Filip Godkjenn.dc.html` | Agent |
...
| 14.5 | **Treningsplanlegger WANG/GFGK Junior** (spec 07.07.2026 — verifiser om fortsatt aktuelt/prioritert før arbeid startes). Tre jobber: **A** — gjør gruppe→spiller-utsending pålitelig. **Status verifisert 02.09.2026:** dedupen i `merge-week-sessions.ts` sjekker allerede `generertFraId` (ikke `id`) — den delen var rettet før; `v2-sync.ts` (atomisk upsert + unik indeks), `duplicate-week.ts` og `duplicate-session.ts` fikk transaksjoner i PR #747 (02.09). **Gjenstår i A:** løkken i `apply-template-actions.ts` (`applyTemplateCore`, `trainingPlanSession.create` + `upsertV2ForPlanSession` per rad + `usageCount`-increment) kjører fortsatt utenfor transaksjon — en feil midt i en mal-utrulling etterlater halve uka; gruppeutrulling er re-kjørbar via G3-dedupen, enkeltspiller-utrulling er det ikke. Krever at `upsertV2ForPlanSession` tar en valgfri tx-klient (`Prisma.TransactionClient`) — liten, men tråd den gjennom med test, ikke i forbifarten. **B** — åpne, delbare gruppe-sider for WANG+GFGK uten innlogging (kun felles gruppedata: tider/kalender/samlinger/turneringer/tester — ALDRI personlige spillerdata, PII-mindreårige-regelen). **C** — koble `gfgkjunior.no` til den datadrevne siden (repoet er ikke lokalisert ennå) |
