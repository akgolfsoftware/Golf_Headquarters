...
| 15.13 | **De 36 skjermene uten vei inn — TO DELER LEVERT 31.08.2026, resten gjenstår** | — | Etter 15.1–15.12: gå gjennom lista i `docs/arkitektur-kartlegging-2026-08-30.md` og gi hver enten en inngang eller en redirect. Ingen skjerm skal stå igjen uten vei inn. **Del 1 (egen PR, PlayerHQ + rydding), etter dokumentets egen prioritet i §8:** (1) fikset den ekte bugen — «be om økt»-innsendingen redirectet til seg selv (`?sent=1`) i stedet for til den ferdigbygde 171-linjers kvitteringen på `/portal/onskeligokt/bekreftet`; ingen andre steder leste `sent=1`. (2) Slettet de tre «aldri gjort ferdig»-flatene dokumentet ba om å fjerne: `/intern/komponenter` (6 filer, koden sa selv «bør fjernes før produksjon»), `/kommando/*` (6 tomme veivisere), `/portal/meg/innstillinger/okter` (ærlig «kommer Q3 2026»-tomtilstand uten data) — pluss komponenten `InnstillingerOkterV2` som ble ubrukt av sistnevnte. Ingen av de tre hadde noen innkommende lenke fra kode. **Del 2 (PR #711) — de 10 AgencyOS-adressene** (PlayerHQ dekket av del 1, ikke del av denne konsolideringen for øvrig): 7 av 10 trengte arbeid — `settings/api`/`settings/security` var allerede løst av 15.3, `wagr-import` fikk sin dør i 15.12. GDPR/eksterne lesere: lenke-rad i Oppsett. Videoer/turnering-kobling: knapper på spillerkortet. AK-stigen: knapp i Stall-hodet (bygget inn i BÅDE vanlig og tom-tilstand — QA fant at tom-tilstanden manglet den, rettet). Kalender-hendelse: lenke i den levende kalenderen (`KalenderLagUkeV2`, den gamle var i død kode). Moderering: sjette Kø-fane. **Gjenstår (verken del 1 eller del 2):** PlayerHQ talent-kjeden (3 skjermer: min-plan/roadmap/sammenligning bør bli faner), SG-hub coach-modus (3, avklar kobles fra Stall eller avvikles), de resterende 8 flyt-endepunktene med samme «redirect til seg selv»-mønster, booking-detaljene (2), og onboarding-duplikatet `/onboard/coach`+`/onboard/klubb` vs. `/auth/onboarding` — **denne siste krever Anders' valg av hvilken som er kanonisk**, ikke en agent-gjetning |
