...
| 0.13 | **21 MARKEDSSIDER LIGGER USTYLET I PRODUKSJON — funnet 31.08.2026** | PR #680 (Paper ut) slettet `src/styles/paper-katalog.css` · `paper-side.css` · `paper-system.css` · `paper-tokens.css` (1 028 linjer), men 21 komponenter i `src/components/marketing/v2/` bruker fortsatt `PkShell` og 75 `.pk-*`-klasser som nå ikke er definert noe sted i `src/`. Verifisert i prod 31.08 på `/junior`: all innholdsstil borte, kun toppbar og bunnfelt står. **Rammet:** `/coaching` `/junior` `/priser` `/om-oss` `/coacher(/[slug])` `/anlegg(/[slug])` `/cases` `/suksess` `/treningsfilosofi` `/faq` `/kontakt` `/turneringer(/[slug])` `/blogg(/[slug])` `/jobb` `/vilkar` `/personvern` `/cookies`. **Fiks:** hent de fire filene fra `git show 8ae2ac47a^`, port `--p-*` → `--mk-*`, gi nytt navn (Paper-vakten `check-ingen-paper.mjs` blokkerer gammelt navn), verifiser alle 21 i 390 og 1440. **Anders 31.08: brandingen (STEG 18) går først, denne tas etterpå** | — | Åpen |
...
| 18.5 | **Logoen — ANBEFALING LEVERT 31.08.2026, VENTER PÅ ANDERS** | Undersøkt: de sju filene i `public/logos/` er ekte vektor med solide former og skalerer rent. Problemet er ikke kvalitet, men FAMILIE. **Anbefaling: rydd, ikke tegn om** — tre nye filer holder: `ak-golf-merke-kvadrat.svg` (profilbilder), `ak-golf-favicon.svg` (ballen forstørret så den overlever 16 px), `ak-golf-laas-<variant>.svg` (logo + virksomhetsnavn låst sammen). Klaringssone og minstemål er nå definert: halve logoens høyde, 24 px / 12 mm. Begrunnelse: `designsystem/ak-golf/guidelines/03-logo.md`. **Anders bestemmer.** Opprinnelig tekst: dagens SVG er 2 kB og tynn i store flater. Avgjør opprydding vs. nytegning, lever full variantfamilie: lys, mørk, monokrom, kvadrat, favicon, klaringssone, minstestørrelse. **Anders bestemmer om den skal tegnes om** |
