...
| OW-3 | **Plan skrevet, venter på Anders (16.09.2026) — ingen kode endret.** Kartlagt: 40+ lesefiler per modell, kun 9/12/21 rader i hhv. `workbench_sessions`/`training_plan_sessions`/`training_sessions_v2` (lav data-risiko, høy kode-risiko). Seks-fase plan med rollback-punkt, anbefaling om at `TrainingSessionV2` består som eget live-lag (re-pekes til `WorkbenchSession` som kilde), og fem åpne spørsmål som må avklares før fase 1–3 starter | [Migreringsplan](planer/ow-3-en-oekt-modell-2026-09-16.md); `prisma/schema.prisma` (6165, 1323, 3642), `src/lib/workbench/wb-actions.ts`, `src/app/portal/planlegge/workbench/actions.ts`, `src/lib/workbench/session-actions.ts` | Én tabell for planlagt økt; `BUSINESS-RULES.md` «Workbench er delt kjerne» stemmer; eksisterende Workbench-tester grønne; datamigrering godkjent særskilt |
