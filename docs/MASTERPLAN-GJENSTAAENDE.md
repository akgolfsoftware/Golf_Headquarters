...
| 15.13 | **De 36 skjermene uten vei inn — DEL 1 LEVERT 31.08.2026, resten gjenstår** | — | Etter 15.1–15.12: gå gjennom lista i `docs/arkitektur-kartlegging-2026-08-30.md` og gi hver enten en inngang eller en redirect. Ingen skjerm skal stå igjen uten vei inn. **Del 1 (denne PR-en), etter dokumentets egen prioritet i §8:** (1) fikset den ekte bugen — «be om økt»-innsendingen redirectet til seg selv (`?sent=1`) i stedet for til den ferdigbygde 171-linjers kvitteringen på `/portal/onskeligokt/bekreftet`; ingen andre steder leste `sent=1`. (2) Slettet de tre «aldri gjort ferdig»-flatene dokumentet ba om å fjerne: `/intern/komponenter` (6 filer, koden sa selv «bør fjernes før produksjon»), `/kommando/*` (6 tomme veivisere), `/portal/meg/innstillinger/okter` (ærlig «kommer Q3 2026»-tomtilstand uten data) — pluss komponenten `InnstillingerOkterV2` som ble ubrukt av sistnevnte. Ingen av de tre hadde noen innkommende lenke fra kode. **Gjenstår (ikke del av denne PR-en):** AgencyOS' 10 dør-løse skjermer (gdpr, videoer, settings/api, settings/security, team/ekstern, agencyos/ak-stigen, spillere/turnering-kobling, kalender/hendelse/ny, stats/moderering — wagr-import fikk sin dør i 15.12), PlayerHQ talent-kjeden (3 skjermer: min-plan/roadmap/sammenligning bør bli faner), SG-hub coach-modus (3, avklar kobles fra Stall eller avvikles), de resterende 8 flyt-endepunktene med samme «redirect til seg selv»-mønster, booking-detaljene (2). **Onboarding-duplikatet LEVERT 31.08.2026:** Anders valgte `/auth/onboarding`
som kanonisk (spurt eksplisitt — de tre alternativene var identiske ADMIN/COACH-grener, ikke
ulike målgrupper). `/onboard/coach` og `/onboard/klubb` (samt deres wizard/actions-filer, ingen
brukt andre steder) slettet og erstattet med redirect-stubs til `/auth/onboarding` |
