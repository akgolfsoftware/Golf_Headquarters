...
11. **Profilkobling til GolfBox-historikk** (`/portal/meg/resultater`) — tre åpne punkter: (a) kontosletting må kalle `dashboard.delete_profile_data(hq_user_id)`, i dag gjør den ikke det; (b) navn og fødselsdato i profilen er selvoppgitt, så koblingen hindrer feilkobling, men er ikke identitetskontroll. Vurder trenerbekreftelse for spillere under 16; (c) GolfBox-vilkårene for bruk av resultatdata er ikke funnet eller verifisert (NGF eier den nasjonale databasen, GolfBox AS er leverandør). Avklar med NGF før profilene deles med Team Norway, WANG eller foreldre.
