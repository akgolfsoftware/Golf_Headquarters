...
| Onboarding (spiller, 5 steg) | `/auth/onboarding` | ✓ | ✓✓– | ~ | ✓ | ✓ | ✓ | v2-port 16. jul (Team B): de DELTE primitivfilene `wizard-chrome`/`wizard-fields` restylet in place til v2 T-tokens (samme eksporterte navn/props — 836-linjers wizard-logikk urørt: mindreårig-sjekk, resume, lagring per steg). Ny `VeiviserFlate`-eksport bevarer den eksisterende lyse flaten. Rettet også rad-tittel: koden har 7 steg, ikke 8. 2026-07-11: fikset lesPreferences-lekkasje (data ble slettet av enhver innstillings-lagring); steg-3-svar (fasiliteter/dager/mål) lagres nå og feeder FacilityPrefs+Goal+plan-engine. 2026-07-17 (PR #42): redusert fra 7 til 5 steg — GolfBox- og TrackMan-auto-connect-stegene fjernet (begge var kun «kommer snart»-knapper uten ekte integrasjon; `getResumeStep()` klemmer nå også resume-stien til gyldig steg).
