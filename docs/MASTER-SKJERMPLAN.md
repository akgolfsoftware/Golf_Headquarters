...
| Mål-hub | `/portal/mal` | ✓ | --- | ✓ | ~ | ~ | ✓ | Reconciliation 16. jul (Fase 0): BEKREFTET — `MalHubV2` inni `V2Shell` (@/components/portal/v2/MalHubV2). **27. jul (mål-progresjon):** fremdrift går nå via delt `beregnGoalProgress` (`src/lib/portal/goals/progress.ts`) — dekker HCP/runder/øktfrekvens/testresultat, viser ærlig «ingen data ennå» i stedet for 0 % der ingenting er logget.
