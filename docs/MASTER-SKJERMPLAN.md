...
- **I arbeid akkurat nå (bygges i natt/parallelt):** ca. 20 — PlayerHQ-skjermene og AgencyOS-skjermene som er nevnt nederst i denne seksjonen.
- **Tegnet ferdig av designer, men venter på å bli koblet til ekte adresse:** mange (de fleste av de 20 i arbeid).
- **Ikke startet:** flertallet av de ca. 180 — de finnes som adresse i appen, men er ikke pusset opp til nytt design ennå.
...
Kort sagt: nesten alt er enten gammelt design som venter på oppussing, eller nytt design som er tegnet men ikke koblet på. Bare Workbench er helt i mål.

### Hva som er bygget i natt (status å sette inn)

Disse ble bygget som nytt design i en forhåndsvisning med liksom-tall i natt. De får derfor:
...
- **PlayerHQ ferdig i natt:** Hjem, SG-Hub, Live-økt (brief / aktiv / oppsummering), Runder.
- **PlayerHQ i arbeid akkurat nå (parallell kjøring):** Statistikk, Analyse, Meg, Abonnement, Drills, Tester, Årsplan, Booking — disse er markert `~ i arbeid`.
- **AgencyOS ferdig i natt:** Cockpit (hjem), Spillere/Stallen, Innboks, Spiller-detalj.
- **AgencyOS i arbeid akkurat nå:** Kalender, Bookinger, Tester, Turneringer — markert `~ i arbeid`.

**Workbench er spesiell:** den eneste skjermen som er helt ferdig hele veien, inkludert ekte data (jobben kalt «W5b» er gjort). Både spiller- og coach-utgaven.
...
| 404 (ikke funnet) | (system) | Designeren har tegnet `mx-404.png` — ikke koblet ennå (se drop-off). |
...
Designeren leverte 44 ferdige skjermbilder. De fleste har nå (eller får i natt) en motpart i appen. Disse har et bilde, men skjermen i appen er enten ikke pusset opp eller ikke koblet:

| Tegnet skjermbilde | Hører hjemme på | Status |
|---|---|---|
| `mx-404.png` (404-side) | Appens «ikke funnet»-side | Ikke koblet. Enkel jobb — bør gjøres. |
| `pl-onboarding.png` | `/auth/onboarding` | Tegnet, men onboarding-skjermen er ikke pusset opp til dette designet ennå. |
| `pl-forelder.png` | `/portal/meg/foreldre` (eller foreldreportalen) | Tegnet, ikke koblet til ekte skjerm. |
| `pl-varsler.png` | `/portal/varsler` | Tegnet, ikke pusset opp ennå. |
| `pl-innstillinger.png` | `/portal/meg/innstillinger` | Tegnet, ikke pusset opp ennå. |
| `pl-trackman.png` | `/portal/mal/trackman` | Tegnet, kun delvis i arbeid. |
| `pl-turnering.png` | `/portal/tren/turneringer` | Tegnet, kun delvis i arbeid. |
| `fo-barn.png` (forelder ser barn) | `/forelder/barn` | Tegnet, ikke pusset opp ennå. |
| `ag-caddie.png` (coach AI-chat) | `/admin/agencyos/caddie` | Tegnet, ikke koblet. |
| `ag-compare.png` (sammenlign spillere) | `/admin/talent/sammenligning` | Tegnet, ikke koblet. |
| `ag-compliance.png` | `/admin/analysere/compliance` | Tegnet, kun delvis. |
| `ag-drift.png` (drift/anlegg) | `/admin/anlegg` / drift-sidene | Tegnet, ikke koblet. |
| `ag-kalender.png` | `/admin/kalender` | Tegnet, kun delvis i arbeid. |
| `ag-tester.png` | `/admin/tester` | Tegnet, kun delvis i arbeid. |
| `mk-forside.png` (marketing-forside) | `/(marketing)` | Tegnet, forsiden er ikke pusset opp til dette ennå. |

> De øvrige skjermbildene (f.eks. `pl-hjem`, `pl-sghub`, `pl-runder`, `pl-live-*`, `ag-dashboard`, `ag-stallen`, `ag-innboks`, `ag-spiller`, `ag-workbench`, `au-login` m.fl.) er allerede tatt i bruk eller under bygging i natt — de er IKKE drop-off.
...
**Bolk 1 — Gjør ferdig det som bygges i natt (ingen nytt design trengs).**
De ~20 skjermene som er tegnet og under bygging (PlayerHQ-hjem, SG-Hub, Live-økt, Runder, Statistikk, Analyse, Meg, Abonnement, Drills, Tester, Årsplan, Booking + AgencyOS cockpit, Spillere, Innboks, Spiller-detalj, Kalender, Bookinger, Tester, Turneringer): flytt dem fra forhåndsvisning til ekte adresse, koble på ekte data, og test. Mål: alle seks haker grønne.

**Bolk 2 — Plukk de enkle drop-off-skjermbildene (kan bygges selv).**
404-siden, varsler, innstillinger, onboarding, marketing-forside, forelder-barn. Disse er tegnet og venter — bare å koble på.
