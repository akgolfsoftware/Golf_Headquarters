...
| Skjerm | Adresse | Design | Mob/Desk/iPad | Adresse-ok | Flyt | Data | Funker |
|---|---|---|---|---|---|---|---|
| Planlegge (= Workbench mobil) ★ | `/portal/planlegge` | ✓ | ✓✓✓ | ✓ | ✓ | ✓ | ✓ | 2026-08-03 designport steg 7 PR2: bygget om mot Paper-fasiten (`playerhq-plan.html`) — interaktiv dag-navigasjon (`DagStripe` som velger), «hvorfor dette tallet»-utvidelse på uke-%, eierskaps-melding, «Book coachingtime»-CTA, og øktdetalj i `BunnArk` (målsetning + drills) ved tap på en økt. `getDashboardData` utvidet med `sted`/`drills`/`maalsetning` for hele uka, ikke bare i dag. Flytting av økt skjer bevisst fortsatt kun i Workbench (lenket herfra) — ingen ny mutasjonsvei bygget i denne PR-en. `npm run verify` + 903/903 tester grønt.
...
| Årsplan | `/portal/tren/aarsplan` | REDIRECT | ✓✓– | ✓ | ~ | ✓ | ✓ | Redirect-stubbe → /portal/planlegge/workbench?zoom=ar (triage 2026-08-04) — ikke en skjerm |
...
| · Fys-plan detalj/bygger | `/portal/tren/fys-plan/[planId]` | REDIRECT | --- | ✓ | ~ | ~ | ✓ | Redirect-stubbe → /portal/fysisk (triage 2026-08-04) — ikke en skjerm |
...
| · Milepæler | `/portal/mal/milepaeler` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/talent (triage 2026-08-04) — ikke en skjerm
...
| · Turnering-detalj | `/portal/tren/turneringer/[id]` | ✓ | ✓✓– | ✓ | ✓ | ~ | ✓ | v2-port 17. jul (Team D1): `TurneringDetaljV2` (V2Shell/Kort/StatusPill/TomTilstand) erstatter hybrid-designet, ruten flyttet ut av (legacy). `loadTurneringDetalj`-loaderen og server actions (`meldDegPa`/`meldDegAv` fra (legacy)/tren/turneringer/actions.ts — beholdt der, deles med lista) uendret. Not-found beholdt med ærlig tomtilstand. **Live turneringsrunde (D6c) 17. jul:** «Start turneringsrunde»-CTA (for påmeldte, når banen er kjent) → `startTurneringsrunde` oppretter `Round(roundType="turnering", tournamentEntryId)` → hull-for-hull-flaten; fortsetter eksisterende runde i stedet for dublett. Design – → ✓, Adresse ~ → ✓, Flyt → ✓, Funker → ✓. |
| · Ny turnering | `/portal/tren/turneringer/ny` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/tren/turneringer (triage 2026-08-04) — ikke en skjerm
...
| · Ny utfordring (wizard) | `/portal/utfordringer/ny` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/utfordringer (triage 2026-08-04) — ikke en skjerm
...
| Ny økt (handlingsvalg) | `/portal/ny-okt` | REDIRECT | --- | ✓ | ✓ | ✓ | ✓ | Redirect-stubbe → /portal/planlegge/workbench (triage 2026-08-04) — ikke en skjerm
...
| Økt-detalj | `/portal/tren/[sessionId]` | REDIRECT | --- | ✓ | ~ | ~ | ✓ | Redirect-stubbe → /portal/live/[sessionId] (triage 2026-08-04) — ikke en skjerm
...
| **SG-Hub (Strokes Gained)** ★ | `/portal/mal/sg-hub` | REDIRECT | ✓✓– | ✓ | ✓ | ✓ | ✓ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Kølle-detalj | `/portal/mal/sg-hub/[club]` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Benchmark | `/portal/mal/sg-hub/benchmark` | REDIRECT | --- | ✓ | ~ | ✓ | ✓ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Best vs nå | `/portal/mal/sg-hub/best-vs-now` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Utstyr | `/portal/mal/sg-hub/equipment` | ✓ | ✓✓– | ✓ | ~ | ~ | ~ | v2-port 17. jul: `UtstyrHelseV2` (Kort/Caps/Tittel/StatusPill/TomTilstand + T-tokens, HjelpTips på smash factor/TrackMan) erstatter rå Tailwind i `EquipmentView`. ALL datalogikk (prisma-query + `computeClubFit`) uendret server-side — komponenten tar ferdig utregnede `ClubFitReport[]`. Deles med coach-inngangen (`coach/[spillerId]/equipment`), som arver porten automatisk. Mob/Desk/iPad --- → ✓✓–. |
| · Avstander (yardage) | `/portal/mal/sg-hub/yardage` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Forhold (vær/bane) | `/portal/mal/sg-hub/conditions` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
| · Strategi | `/portal/mal/sg-hub/strategy` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/sg-hub (triage 2026-08-04) — ikke en skjerm |
...
| · Coach: utstyr | `/portal/mal/sg-hub/coach/[spillerId]/equipment` | ✓ | ✓✓– | ✓ | ~ | ~ | ~ | 17. jul (Team F3): wrapper-ruten flyttet ut av (legacy) (samme URL, samme auth som naboruten, nå V2Shell-chrome). v2-port fullført 17. jul: `EquipmentView` bruker nå `UtstyrHelseV2` → coach-inngangen arver v2-visningen automatisk (delt visning med spillerens equipment-side). Design – → ✓, Mob/Desk/iPad --- → ✓✓–. |
| Runder (liste) | `/portal/mal/runder` | ✓ | ✓✓– | ✓ | ~ | ✓ | ✓ | Design rettet – → ✓ 16. jul: `V2Shell` + `RunderV2`. |
| · Runde-detalj ★ | `/portal/mal/runder/[id]` | ✓ | ✓✓– | ✓ | ✓ | ✓ | † | D6a 17. jul: «Fra scorekortet»-kort (UT/INN/TOTALT m/ tonet ±, putter/FW/GIR-aggregater m/ HjelpTips — kun fra ekte HoleScore-rader) + «Rediger hull-for-hull»-lenke + ærlig tomtilstand m/ «legg til nå»-CTA. **GO V2 (24. juli):** «Runden er lagret» er nå ren kvittering — konkurrerende import/detalj-knapper fjernet fra kortet (finnes fortsatt i sekundærraden), så CTA-pilla under er den ene neste handlingen. |
...
| · Fullfør kjeden (import/hurtig → slag-kjede per hull) ★ | `/portal/mal/runder/[id]/fullfor` | REDIRECT | --- | ✓ | ✓ | ✓ | ~ | Redirect-stubbe → /portal/mal/runder/[id] (triage 2026-08-04) — ikke en skjerm
...
| · Live slag-for-slag-føring ★ | `/portal/runde/live` | ✓ | ✓✓– | ✓ | ✓ | ✓ | ✓ **GO V2 (24. juli):** tommel-sone — primærhandlingen («Lagre hull»/«Neste hull»/«Lagre runden») klistres nederst i skjermen (sticky + safe-area) via delt `TommelSone`/`PrimaerKnapp`; sekundær navigasjon (Hull-oversikt, SG hittil, Tilbake) flyttet over sonen. |
| · Etterregistrering slag for slag ★ | `/portal/runde/logg` | ✓ | ✓✓– | ✓ | ✓ | ✓ | ✓ **GO V2 (24. juli):** samme tommel-sone som live-føringen (delt `tommel-sone.tsx`). |
...
| · Test-katalog (NGF) | `/portal/tren/tester/katalog` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/tren/tester (triage 2026-08-04) — ikke en skjerm
...
| ~~· Test live (fullskjerm)~~ | `/portal/(fullscreen)/test/[testId]/live` | — | — | — | — | — | — | RUTE FINNES IKKE i koden, og **ingen lenker peker på den** (verifisert 2026-07-18). **UTSATT POST-LANSERING (Anders 2026-07-18):** ingen lanseringssperre — bygges eventuelt senere. |
| ~~· Test oppsummering~~ | `/portal/(fullscreen)/test/[testId]/summary` | — | — | — | — | — | — | Samme — rute finnes ikke, ingen lenker. UTSATT POST-LANSERING (Anders 2026-07-18). |
...
| Statistikk-side (gml.) | `/portal/mal/statistikk` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/analysere (triage 2026-08-04) — ikke en skjerm
...
| · Coach-profil | `/portal/coach/[coachId]` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach (triage 2026-08-04) — ikke en skjerm |
...
| Meldinger (innboks) | `/portal/coach/melding` | ✓ | ✓✓– | ✓ | ~ | ~ | ✓ | **M3 mobil (bølge 3) 17. jul:** ekte master/detail på mobil — trådliste full bredde, tapp på innlastet tråd → tråd-visning m/ tilbake-lenke; tommelvennlig komponer-felt (`Skrivefelt`, sticky bunn + safe-area) med raske svar-maler (`ForslagRad`, copy fra `CoachMeldingNyV2`s `HURTIGVALG`). Desktop side-om-side uendret. Merknad: skjerm-bunn-festet komponer over shell-bunn-nav vil kreve en delt offset-token i shell (meldt gap, ikke improvisert). Mob --- → ✓✓–. |
| · Ny melding | `/portal/coach/melding/ny` | ✓ | --- | ✓ | ✓ | ✓ | ✓† |
| · Meldingstråd | `/portal/coach/melding/[id]` | REDIRECT | --- | ✓ | ✓ | ~ | ✓ | Redirect-stubbe → /portal/coach/melding (triage 2026-08-04) — ikke en skjerm |
| · Vedlegg | `/portal/coach/melding/[id]/vedlegg` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/melding (triage 2026-08-04) — ikke en skjerm |
| Coach-planer | `/portal/coach/plans` | ✓ | --- | ✓ | ~ | ~ | ~ |
| · Plan-detalj | `/portal/coach/plans/[planId]` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/plans (triage 2026-08-04) — ikke en skjerm |
| · Ny økt i plan | `/portal/coach/plans/[planId]/ny-okt` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/planlegge/workbench (triage 2026-08-04) — ikke en skjerm |
| · Perioder | `/portal/coach/plans/perioder` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/planlegge/workbench?zoom=ar (triage 2026-08-04) — ikke en skjerm |
| Coach-øvelser | `/portal/coach/ovelser` | ✓ | --- | ✓ | ~ | ~ | ~ |
| · Ny øvelse | `/portal/coach/ovelser/ny` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/ovelser (triage 2026-08-04) — ikke en skjerm
| · Rediger øvelse | `/portal/coach/ovelser/[id]/rediger` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach/ovelser (triage 2026-08-04) — ikke en skjerm |
| Coach-videoer | `/portal/coach/videoer` | ✓ | --- | ✓ | ~ | ~ | ~ |
| Coach-notater | `/portal/coach/notes` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach (triage 2026-08-04) — ikke en skjerm |
| · Notat-detalj | `/portal/coach/notes/[noteId]` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal/coach (triage 2026-08-04) — ikke en skjerm |
...
| Skjerm | Adresse | Design | Mob/Desk/iPad | Adresse-ok | Flyt | Data | Funker |
|---|---|---|---|---|---|---|---|
| Booking-hub | `/portal/booking` | ✓ | ✓✓– | ~ | ~ | ✓ | ~ | Ombygget 2026-08-04 (Anders' instruks): var `BookingV2`-veiviseren montert direkte på hub-ruten (stepper først, ingen oversikt). Nå `BookingHubV2` — default-visning er timer/credits-status («X av Y timer igjen», StatusPill) + kommende bookinger + coach-liste, FØR noen veiviser. `getBookingHubData` ble hentet her fra før men kun credits+coacher ble vist — `upcoming` var hentet og aldri rendret; nå brukt fullt ut, ingen ny query. Selve bookingen skjer fortsatt på `/portal/booking/ny` (uendret, se raden under) — denne siden dupliserer ikke den credits-bevisste logikken. `BookingV2` (stepperen) rørt IKKE — brukes fortsatt av den offentlige `/booking` og av `AdminUkaV2`. tsc/eslint/903 tester/build grønt lokalt. Funker ✓ → ~: ny implementasjon, ikke click-testet i nettleser med ekte data ennå (kun bygg-verifisert) — se PR. |
...
| Reach (oppsøk-verktøy) | `/portal/reach` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal (triage 2026-08-04) — ikke en skjerm
| Agent-pipeline (AI internt) | `/portal/agent-pipeline` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /portal (triage 2026-08-04) — ikke en skjerm
...
| · Spillere (snarvei) | `/admin/agencyos/spillere` | REDIRECT | ✓✓– | ✓ | ✓ | ✓ | † | Redirect-stubbe → /admin/spillere (triage 2026-08-04) — ikke en skjerm |
...
| Oppfølging (alias → queue) | `/admin/oppfolging` | REDIRECT | ✓✓– | ✓ | ✓ | ✓ | † | Redirect-stubbe → /admin/queue (triage 2026-08-04) — ikke en skjerm |
...
| Grupper | `/admin/grupper` | ✓ | –✓– | ✓ | ✓ | ✓ | † | Design rettet – → ✓ 16. jul: `V2Shell` + `GrupperV2`. PR #44 (17. jul): «Ny gruppe» koblet til ekte `createGroup`-action via modal (`ny-gruppe-modal.tsx`) — oppretter og navigerer til ny gruppe; tsc/build grønt, ikke nettleser-testet ennå (Funker ✓ → †). |
| · Gruppe-detalj (+ VG-trinn filter/badge, 2026-07-07) | `/admin/grupper/[id]` | ✓ | ✓✓– | ✓ | ✓ | ✓ | †  PR #44 (17. jul): 2026-07-16: «Slett gruppe» lagt til (`SlettGruppeButton`/`deleteGroup`) — kaskade-sletter GroupMember/GroupSchedule, krever bekreftelse med antall medlemmer/samlinger i UI |
...
| · **WANG Toppidrett Fredrikstad — Golf-fellesside (v2-redesign)** — egen WANG-merkevare (navy/teal/mint, Montserrat + Quattrocento, ekte våpenskjold), scoped tokens `src/styles/wang-tokens.css`, `noindex`. Faner: Oversikt · Plan (Sesong: årshjul + tidslinje) · Skole/Foreldre (bygges) | `/team-wang` | ✓ | ✓✓– | ✓ | ✓ | ✓ | † | 19. jul: komplett rebuild fra Claude Design «WANG Toppidrett Fredrikstad Golf v2». Hardkodet sesongplan 2026/27 (80KB datamodell: perioder, øktmaler per VG, AK-drills, turneringer, tester, skole, samlinger, kompetansemål). Oslo-korrekt dato klient-side. Erstatter den gamle DB-kalenderen (gruppe-kalender-komponentene beholdt for /gfgk-junior/kalender). Skole/Foreldre + Plan/Kalender/Samlinger gjenstår. |
...
| · Kohort | `/admin/talent/kohort` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /admin/talent/radar (triage 2026-08-04) — ikke en skjerm |
| · Region | `/admin/talent/region` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /admin/talent/radar (triage 2026-08-04) — ikke en skjerm |
| · Ressurser | `/admin/talent/ressurser` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /admin/talent/radar (triage 2026-08-04) — ikke en skjerm |
...
| · WAGR-benchmark | `/admin/talent/wagr-benchmark` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /admin/talent/radar (triage 2026-08-04) — ikke en skjerm |
...
| · Per spiller | `/admin/teknisk-plan/[spillerId]` | REDIRECT | --- | ✓ | ~ | ~ | ~ | Redirect-stubbe → /admin/spillere/[spillerId]/plan (triage 2026-08-04) — ikke en skjerm |
...
| Kalender | `/admin/kalender` | ✓ | ✓✓✓ | ✓ | ✓ | ✓ | ✓ | 2026-07-24 Bølge 5: TrainingSessionV2-blokker med drill-lesevisning (BunnArk: planlagt/gjort reps per drill). M3 mobil liste-først uendret. **GO V3 (24. juli):** `SerieMeny` kjører på `BunnArk`-kontrakten (fokus-felle, fokus-gjenoppretting, scroll-lås, Escape, sheet-in) — ad-hoc overlay fjernet. |
...
| Tester (på tvers) | `/admin/tester` | ✓ | ✓✓– | ✓ | ✓ | ✓ | ✓ | Design rettet – → ✓ 16. jul: `V2Shell` + `AdminTesterV2`. **M3 mobil (bølge 5) 17. jul:** `useMobile()` — komprimert tittel, resultat-lista dropper indre scroller på mobil (ingen dobbel-scroll); KPI alt 2-kol, resultater alt `Rad` m/ `DeltaChip`. |
...
| Godkjenninger | `/admin/godkjenninger` | ✓ | ✓✓– | ✓ | ✓ | ✓ | ✓ | 2026-07-14 dok-verifisering: `AdminGodkjenningerV2` — én kø samler PlanAction (agent-forslag) + CaddieDraft (AI-utkast) + SessionRequest (økt-forespørsler) = **3 kilder** (e-postutkast beholder bevisst egen godkjenning i `/admin/innboks-epost` — ikke en 4. kilde i denne køen), gruppert per spiller, paginert, screenshot-verifisert 1440+390. Design rettet – → ✓, Mob/Desk/iPad –✓– → ✓✓–. **M3 mobil (bølge 5) 17. jul:** polert mobil-gren — tettere kort-padding + diff-tekst brytes (`overflow-wrap`) i stedet for å flyte utenfor; handlingsknapper alt 44px fullbredde. **GO V3 (24. juli):** primær CTA følger køen — «Ta N saker som haster» når noe haster, ellers masse-godkjenn lav-risiko; med vanlig kø er kortets Godkjenn eneste primær (innboks blir tekstlenke). |
...
> /admin/godkjenninger. Fullt skjerm-/funksjonsinventar med duplikat-analyse: git-historikken.
>
...
> deres rekomponeres fortsatt bølgevis per git-historikken.
...
**Bolk 6 — Data-blokkert · DELVIS LUKKET 2026-07-17 / rest 2026-07-24.**
- ~~Shot-map / spredning~~ **LUKKET (D6b):** `CourseMap` i slag-wizard der `CourseDefinition.geojson` finnes.
- ~~Scorecard hull-for-hull~~ **LUKKET (D6a):** `HoleScore` + rediger-flate `/portal/mal/runder/[id]/hull`.
- ~~Live turnerings-tracking (spiller hull-for-hull)~~ **LUKKET (D6c):** `Round.tournamentEntryId` + CTA.
- **Gjenstår (D8):** ekte per-hull-geometri i hull-analysen — **blokkert på datakilde** (manuell / GolfBox / generering). Sanntids turnerings-leaderboard er bevisst utenfor D6c.
...
- 4. august (Fase 1-planlegging, rydding av stale rader): **37 rader merket REDIRECT**
  (30 portal + 7 admin) — verifisert mekanisk mot `scripts/paper-port-triage.mjs`-kjøring
  + grep av selve `page.tsx`-filene: rutene er 4–20-linjers redirect-stubber, ikke skjermer.
  34 stub-ruter var allerede korrekt merket; 12 stub-ruter har ingen rad her (bl.a.
  `/portal/baneguide`-familien og `/admin/agenter|ai|mer|prosjekter|risiko|uka`) og skal
  heller ikke få det. REDIRECT-rader teller ikke i porteringsomfanget for steg 7–9.
...
- 3. august (forenkling bølge 2 — pekere og dokument-råte): **Ingen skjermendringer.**
  ...
  git-historikken — **denne planen er eneste levende skjermkilde.**
...
- 24. juli: **GO V2 Live + runde** — delt tommel-sone (`src/components/portal/runde-logg/tommel-sone.tsx`) holder primærhandlingen nederst i skjermen i både slag- og hurtigmodus og i oppsummeringen; live-økt-oppsummeringen og «Runden er lagret» har én neste handling hver. Se git-historikken §4.

- 24. juli: **GO V3 Coach-dag først** — cockpit NÅ øverst (live → AI-dispatch → Triage før KPI/fokus/innboks); Stall rad = detalj + Workbench som CTA + «trenger deg» sortert øverst; Godkjenninger primær følger køen (haster → lav-risiko → tom); `SerieMeny` → `BunnArk`-kontrakt. Se git-historikken §4.

- 24. juli: **GO V1 Dommer-finpuss** — Analysere default SG + «Logg runde» + ordbok uten OTT/APP/ARG-koder; Hjem én primær CTA (Workbench ghost når ingen økt); Plan HjelpTips på uke-%/SG + tom-uke ghost-CTA. Se git-historikken.

- 24. juli: **Ferdigstillingsplan Fase A+B levert (gren).** Dokumentsynk (STATUS-NÅ,
  skjermplan-master bølger (dokumentet slettet 05.08, se git), Bolk 6); `lastLoginAt` på alle innloggingsstier +
  `scripts/send-aktiverings-epost.ts` + push-opt-in på PlayerHQ-hjem;
  lanserings-smoke e2e; live offline-kø for drill-reps + spiller-vurdering
  write-back; nivåplasserings-quiz i onboarding (6 steg); Bølge 5 treningsanalyse
  + kalender drill-lesevisning; soft-haker på ★-kjerne. Se `docs/LANSERING-P0-ANDERS.md`.
...
- 19. juli (illustrativt sone-kart bygget, etter Anders' svar på formell bestilling): **drop-off-
  ...
  designbestillingen for sonekart (slettet 05.08, se git) fikk to avklarte svar (1: illustrativt
...
- 16. juli (Byggerunde 1a-verifisering + reell temabug, branch `claude/reskin-auth-screens`):
  ...
  fra denne tabellen.
- 16. juli (`/kommando` fjernet, B8 i git-historikken, branch
...
- 14. juli (ren dokument-verifisering — 7 punkter fra intern oppgavelogg sjekket mot faktisk
  ...
  og den ovennevnte Ny spiller-siden — resten av P1-lista i git-historikken
  ...
  git-historikken. tsc/build ikke kjørt (ren dokument-endring).
...
- 14. juli (komplett prosjekt-revjuw og opprydding, branch `claude/prosjekt-opprydding`):
  ...
  **Docs:** 4 punkt-i-tid-rapporter arkivert (git-historikken oppdatert), 4 manglende
...
Full kronologisk byggehistorikk flyttet til git-historikken
