...
- **Oppgave 6.4 og 6.6 må kjøres fra hovedmappa/maskinen med `.env.local`** (`SCREENTEST_PASSWORD`).
  ...
  finner likevel binæren.
- GitHub-secreten finnes allerede — ingen `gh secret set`: `gh secret list | grep SCREENTEST` →
  `SCREENTEST_PASSWORD 2026-08-26T20:01:54Z`. Planen «Komplett designport» §4 pkt. 6 sier «ny secret» — det stemmer
  ikke; `scripts/roter-screentest-passord.ts` (steg 4) setter den ved hver rotasjon. Alle tre testbrukerne deler
  passordet (`roter-screentest-passord.ts:23–29`): `screentest@akgolf.test` (PLAYER), `coachtest@akgolf.test`
...
`process.exit()` ni steder, ingen `export`. Spec-en kan ikke importere den. Motoren flyttes ut i en modul uten
...
- [ ] **Steg 1: Stå riktig.** `cd ~/Developer/akgolf-hq && git checkout claude/fase1-okt6-nattlig-maaling` (hovedmappa har
  `.env.local`; `grep -c SCREENTEST_PASSWORD .env.local` → `1` — les aldri verdien). Er grenen bare i worktreen, hent
  den: `git fetch origin && git checkout -t origin/claude/fase1-okt6-nattlig-maaling` etter `git push -u origin HEAD` fra worktreen.
...
- [ ] **Steg 3: Den nye jobben.** Legg til på slutten av fila (etter linje `          echo "Telegram-varsel sendt."`, siste linje), med ett tomt skille:
  ...
  Har du `actionlint` (`which actionlint`): kjør `actionlint .github/workflows/playwright.yml` → stille. Har du det ikke, er
  `gh workflow run` i 6.6 den ekte sjekken.
...
- [ ] **Steg 3: Første kjøring i CI — fra grenen, før merge.** `workflow_dispatch` kan kjøre workflow-fila slik den ligger på grenen:
  ...
  **Rød på pixelnærhet i CI men grønn lokalt = font-gapet** (README-avsnittet over): les annotasjonene `avvik` i
  ...
  Bruk `mcp__github__pull_request_read` med `get_status` for status, ikke `actions_list` (gotchas.md §GitHub PR-overvåking).

- [ ] **Steg 4: Merge og rydd.**
  ```bash
...
- [ ] **Steg 5: Øktslutt.** Én linje i `docs/feillogg.md` (format på linje 3), f.eks.
  `2026-09-06 | ren økt` — eller hva som kostet tid (typisk: forelder-brukeren manglet i prod; CI-font-gapet var større enn +5 for én rad).
  ```bash
...
8. `git log origin/main --oneline -1` viser PR-en merget; `git branch -r | grep -c okt6-nattlig` → `0`.
...
11. **Planen «Komplett designport» §4 pkt. 6 sier «ny secret `SCREENTEST_PASSWORD`»** — den finnes siden 26.08.2026
    (`gh secret list`). Én setning å rette i plandokumentet (docs-only, økt 8 eller når planen oppdateres).
