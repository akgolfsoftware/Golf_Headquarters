...
- Dagens dato 2026-09-06. `origin/main` = `c1c3396eb` (PR #787, #788 og #789 er alle MERGET —
  #789 06.09 09:24 UTC; verifisert med `gh pr view 789 --json state`). Ingen av dem rører filene i
  denne økta (`gh pr view 787 --json files` osv.: kun docs/rigg, `WorkbenchV2*.tsx` og Min kurve).
- **Jobb aldri i hovedmappa `~/Developer/akgolf-hq`.** Den står på den lokale grenen
  ...
  `.claude/hooks/env-lenke.mjs` lenker inn miljøet — kopier ALDRI `.env*` selv).
...
Linjenumrene under er målt på `origin/main @ c1c3396eb`. Har docs-commiten fra 05.09 (STEG 20, kø 31–32)
kommet inn før deg, ligger MASTERPLAN-radene 4 linjer lenger ned (728/729) — **finn alltid raden med grep**,
ikke med linjenummer. Teksten i radene er verifisert identisk i begge versjoner.
...
- [ ] **Steg 8: STATUS-NÅ.** Fem steder (linjer på c1c3396eb; bekreft med grep før du redigerer):
  ...
  Verifiser: `grep -n "19.6/19.7" docs/STATUS-NÅ.md` → kun linje 265 og 271 igjen (revisjonshistorikk 05.09 — la stå).
...
3. **Docs-commiten 05.09 i worktreet `design-implementation-status-14d2ce`** (`ae73b1658`, ikke på
   `origin/main`) legger inn to lenker til `docs/superpowers/plans/2026-09-05-designport-fase-1.md`
   (i `beslutninger.md` og MASTERPLAN) — fila finnes ikke, og `node scripts/check-doc-lenker.mjs` gir
   «2 død(e) doc-lenke(r)» der. Kommer den commiten inn før økt 2, blir `npm run verify` rød for alle.
   Tilhører økten som skrev den (opprett fila, eller pek på `2026-09-05-komplett-designport.md`).
