...
- Aldri `git add -A`. Stage navngitte filer. Ingen nye avhengigheter (`git`, `tar` og Node-innebygde moduler holder — `tar` verifisert på macOS 06.09, ubuntu-latest har den). Ingen nye tokens. Aldri emoji. Norsk bokmål i kommentarer.
...
- [ ] **Steg 3: Skriptet.** Opprett `scripts/check-fasitdekning-baseline.mjs` (kjørt og verifisert 06.09: exit 0 ved likt tall, exit 0 med «baseline kan heves»-melding når HEAD er høyere, exit 1 når baselinen er satt kunstig høyt, `eslint --max-warnings 0` grønn, ingen `git`-kall):
  ```js
  #!/usr/bin/env node
  /**
  ...
  ```
  Merk: `main()` kjører bare når fila startes direkte, ikke når testen importerer den. `lesBaseline` kaller `process.exit(2)` direkte (ikke kastet feil) fordi en manglende/ugyldig baseline-fil er en oppsettsfeil, ikke et dekningsbrudd — samme skille som exit 1 vs. 2 hadde i den forrige (git-baserte) varianten.
