...
URL-bevaring, åtte formelfelt med hjelp og fast mobilpanel er kontrollert
innlogget. Den ekstra datonavigasjonen og tabelloverskriften er fjernet for å
følge valgt design. «Publiser stall» bruker eksisterende bekreftelsesdialog og
atomisk publisering; dialogen er prøvd med tre syntetiske utkast uten skriving.
...
overstyres. Ny PNG-kontroll målte 6,555 % desktop og 7,842 % mobil. Hele
testsuiten består med **3341 tester og 4 komponenttester**; ny full verify er
midlertidig sperret av en annen oppgaves `outputs/` i prosjektroten.
...
**Min kalender-kontroll 21.09:** Min kalender er koblet til den beskyttede
spilleruten med coachens egne økter, maler og bookinger i Oslo-tid. Mobilens
dagbytte, valg av booking, detaljpanel og riktig lenke er prøvd innlogget i det
isolerte miljøet. Skjermen har ingen Publiser-handling. Ny PNG-kontroll målte
5,892 % desktop og 6,708 % mobil. Typesjekk, lint, 17 målrettede tester,
token-, signalfarge- og kildekontroll består. Se
[Min kalender-kontrollen](../design-audit/workbench-min-kalender-kontroll-2026-09-21.md).
Alle åtte Workbench-visninger er dermed lokalt implementert og kontrollert.
...
Uke, Periode, Måned, År, Økt, Stall, Live og Min kalender er lokalt implementert
og kontrollert innlogget mot isolert database; produksjonsversjonen er ikke
verifisert med den nye koden. Neste produktflate er PlayerHQ etter valgt design
i den separate designoppgaven. Siste bevis og testresultater står i de daterte
rapportene under `docs/design-audit/`.
...
| Min kalender | Importert | Implementert lokalt | Kontrollert desktop/390 | Gjenstår |
