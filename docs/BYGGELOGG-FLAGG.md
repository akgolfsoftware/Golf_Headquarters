...
### F-2 · Forelder-undersider — STORT SETT LØST (alle terminal-lys, ærlig data-håndtering)
> **OPPDATERT (verifisert runde 2):** Alle forelder-undersider er nå verifisert terminal-lys + korrekte mobil 430px: barn, samtykke (GDPR-administrasjon), bookinger (read-only), coach (ærlig «kommer Q3 2026»), okonomi (ærlig abonnement+betaling), innstillinger (konto+varsler-status). De var IKKE gammelt hybrid — kun /forelder-hjem var det (nå re-portet). **Betaling LØST:** /forelder/okonomi viser ekte Subscription + ærlig tom betalingsliste + «NESTE TREKK —» uten Stripe-fabrikering (Stripe-kort-display er korrekt N/A, ikke en bug). **Varsler LØST ærlig:** /forelder/innstillinger viser «PÅ E-POST»-status + «individuelle varselbrytere kommer i en senere versjon» — ingen falsk pynt. Eneste reststykke: en DEDIKERT /forelder/varsler med INTERAKTIVE per-toggle-brytere krever fortsatt en NotificationPreference-modell (under). Ikke en blokker — appen håndterer det ærlig i mellomtiden.

Opprinnelig flagg (historikk):
...
- **LØST 2026-06-22 (Anders låste opp):** cockpit-KPIene **STALL-SG** og **PLAN-ADHERENCE** er nå BYGD med ekte data (30d, alle spillere): STALL-SG = snitt sgTotal alle runder siste 30d (`prisma.round.aggregate _avg`, samme def som /admin/runder); PLAN-ADHERENCE = fullført/planlagt TrainingPlanSession siste 30d. Verifisert: viser «+0,8» / «52 %». «—» kun hvis ingen data (ærlig). Kode: daily-brief-data.tsx + agency-cockpit.tsx. Historikk under.
