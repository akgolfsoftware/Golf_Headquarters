...
- **Generer Prisma-klienten i worktreen FØR noe annet.** Bekreftet i denne økten (07.09): en fersk worktree mangler `src/generated/prisma/client` (gitignored, arves ikke), og `npx tsc --noEmit` gir da 2000+ feillinjer over hele repoet («Cannot find module '../src/generated/prisma/client'») — inkludert i filer utenfor det denne økten rører. Det rammer `npx tsc --noEmit`-verifiseringen i oppgave 4.1 steg 5, 4.2 steg 4 og 4.5 steg 7, OG seed-scriptet i 4.5 steg 1 (`seed-screentest-coach.ts` importerer `PrismaClient` derfra og feiler før noe kjører). Kjør derfor FØRST, én gang, kun i skallet (aldri i en fil, aldri kopier `.env*`):
  ```bash
  ...
  ```
  Forventet: `✔ Generated Prisma Client …`. Deretter er `npx tsc --noEmit` stille (verifisert i denne økten: 2045 feillinjer → 0 etter generate). `src/generated/` er gitignored — ikke committ den.
...
- **Riggen kjører mot PROD (`https://akgolf-hq.vercel.app`) med ekte innlogging** (`coachtest@akgolf.test`, passord `SCREENTEST_PASSWORD` i hovedsjekkutens `.env.local`). Worktreen har ingen `.env.local`, og den skal ALDRI kopieres (gotchas). Kjør derfor målingene med hovedsjekkuten som arbeidskatalog og worktreens script i subshell — mønsteret står i oppgave 4.5. Fasitfilene (`designsystem/train-lock`) i hovedsjekkuten er identiske med origin/main (bekreftet på nytt 07.09: `git -C /Users/anderskristiansen/Developer/akgolf-hq diff --quiet origin/main -- designsystem/train-lock` er tom) — det er alt scriptet leser (rute/label kommer som CLI-argumenter, ikke fra fila). `tests/visual/skjerm-mapping.ts` i hovedsjekkuten er derimot IKKE lenger identisk (mangler 55 linjer — radene #787/#789 la til; den grenen ble forgrenet før de ble merget) — uten betydning her.
