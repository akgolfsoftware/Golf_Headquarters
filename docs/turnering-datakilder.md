...
## ✅ VERIFISERT 2026-06-01 — GolfBox JSON-feed bekreftet kjørende

Testet direkte med curl. **Konklusjon: ren JSON-feed finnes, ingen auth, ingen robots-blokk.** Lett tilgang bekreftet (ikke Playwright).

**Lovlighet:** `scores.golfbox.dk/robots.txt` → 404 (ingen restriksjon). `golfbox.no`/`.dk` robots.txt har kun en `Googlebot`-regel (blokkerer `/*?` for Google), ingen `*`-regel som rammer oss. Widget-host er bygd for offentlig tredjeparts-embedding. Defensibel gråsone: offentlig, uten innlogging, robots-ren. Vi holder rate-limit + `AKGolfBot/1.0`-UA.

**Verifiserte endepunkter (host: `https://scores.golfbox.dk`, alle JSON):**
| Handler | Hva | Eksempel |
|---|---|---|
| `ScheduleHandler/GetSchedule/CustomerId/{id}/language/1044` | Terminliste per kunde (sesong→måned→events m/ `ID`, `Name`, `Type`, `StartDate`, `EndDate`) | customerid 1 = Nordic Golf League, 100 = Stavanger GK |
| `CustomersHandler/GetCustomer/...` | Kundeinfo (`Name`, `GolfUnion`, `Nationality`) — filtrer NO/NGF | — |
| `LeaderboardHandler/GetLeaderboard/CompetitionId/{id}/language/2057` | Full leaderboard | felt under `Classes/C{id}/Leaderboard/Entries` |
| `TeeTimesHandler/GetTeeTimes`, `PlayersHandler/GetPlayers`, `OrderOfMeritsHandler/GetOrderOfMerit(s)`, `StatsHandler`, `MatchplayHandler` | startlister, felt, sammenlagt, stats | — |

**Leaderboard-entry gir nøyaktig ønsket dybde** (verifisert mot ekte data — INFINITUM Championship, 11 norske i feltet):
- `Position.Calculated` ("1", "T3"), `ScoringToPar.ToParText` ("-23"), `.TodayText` ("-11"), `.HoleText` ("F(1)" = thru/ferdig)
- `FirstName`/`LastName`, `Nationality` ("NO"), `Country`, `BirthYear`, `ClubName`
- `Rounds[]` (runde-for-runde) + `RoundNames` (["R1","R2","R3"]) + `ActiveRoundNumber`
- Bonus: `Wagr`-felt på hver entry (WAGR-tellende kan leses direkte)

**Enumererings-strategi:** hver klubb OG hver tour = numerisk `CustomerId`. Iterer customers → `GetCustomer` (filtrer `Nationality=NO`/`GolfUnion=NGF`) → `GetSchedule` → competition-IDer → `GetLeaderboard`. Hele norsk + nordisk golf er dermed enumererbart fra én feed.

**Parsing-detalj:** responsene bruker JS-literaler (`!0`/`!1`). Pre-prosesser `!0→true`, `!1→false` før `JSON.parse` (trivielt).

**Dekker direkte:** alt norsk (NGF/Olyo/Srixon/Garmin NC/regioner), Nordic Golf League, og — via samme plattform — R&A- og EGA-events. GJGT (globaljuniorgolflive.com) er separat.
