...
| PX-6 | AgencyOS-resten mot Train-lock — SAMMENSLÅTT av to parallelle økter (`claude/px6-agency-rest-fasit-iopubc` + `px/6-agency-rest`). Fasit-sitering for cockpit-varianter, AgenticOS-flatene, live-tavle, kalender-lag, økonomi, og PlayerHQ-restene (booking, coach-hub, gameplan, Meg-undersider). AG-11 tidslinje-layout flyttet til domain med enhetstester. Ved konflikt beholdt `claude/px6`-varianten — den gjorde mest arbeid og bruker Train-lock-terminologi, mens den andre stedvis hadde «Presis» (avviklet, invariant 2). BO-01-henvisningen på BookingHubV2 hentet fra den andre grenen. | `samle/px6-agency` | Åpen PR — skjermbilder IKKE tatt |
| PX-7 | Tilstander/brekk — SAMMENSLÅTT av to parallelle økter (`px/7-tilstander-brekk` + `claude/px7-tilstander-brekk-4cp4nu`, se PR). B1 laster/feil for I dag/Plan/Analyse/Meg (skeleton-varianter + fasit-kopi i `V2Feil`/`V2Laster`), GAP-1 tom/laster/feil portet til KA-01/RU-01/S3-01/BO-01, GAP-2 Jarvis-kø-tom, GAP-00 + MAT-00/01 sitert som referanse, B3/B4 lys verifisert. Ved konflikt i delte filer beholdt `px/7`-varianten; unike filer fra begge grener er med. B2 (iPad/Mac-brekk) spec'et, ikke bygget. | `samle/px7-tilstander` | Åpen PR, DELVIS — skjermbilder IKKE tatt |
