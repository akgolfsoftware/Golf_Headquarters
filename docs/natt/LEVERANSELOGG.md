...
| PX-3 | Pikselport TM-familien (delvis): hullkart-bakgrunn (ny `HoleMap.tsx`), slag-ark, «Funn»-liste, lavt-n caddie-setning, TE-04-token-fiks. Fasit-dekning 105→113/204. Se `docs/natt/PIKSELPLAN-2026-08-28.md` §7 | `claude/project-status-screens-smffo7` | PR #656 åpen (draft) — skjermbilder IKKE tatt (ingen DB/Supabase-credentials i økten) |
| PX-3-rest | TE-hub-restrukturering (TE-01/01L): flat GOLFSLAG/TEKNIKK-gruppert liste erstatter pyramide-akse-hero (ny `hub-gruppe.ts`), ekte forfall-caps fra `TestAssignment.dueDate`. TE-03 (Gate-detalj, dekker alle 5 type A-protokoller): giant hero + «Siste forsøk» OK/BOM·V\|H fra ekte `TestResult.details` — fant og fikset en reell les/skrive-skjema-drift (`ScoringDetailsSchema` tillot ikke miss_side «V»\|«H» som scoreTest() faktisk skriver). TM-03/12/13/14 og TE-07/08/09/10/12 vurdert og dokumentert IKKE porterbare uten ny funksjonalitet/datamodell (anti-scope) — se `docs/natt/PIKSELPLAN-2026-08-28.md` §7 og `docs/feillogg.md` 29.08. Fasit-dekning 113→115/204 | `claude/project-status-screens-smffo7` | PR #656 åpen (draft) — skjermbilder fortsatt IKKE tatt (samme credential-mangel) |
