...
| PX-6 | AG (4) + AO (5) + JV (3) + S3 (3) + KA (3) + EC (2) + DG/FY/TU/GP/BO/ME-rest | ~28 | **KJØRT 29.08.2026** (`claude/px6-agency-rest-fasit-iopubc`). Sitert/portet 19: AG-01 lys + AG-09 (sitering — allerede TL, `(+lys)`-kortformen matchet ikke scriptet), **AG-11 Kalender dag bygget som ny `DagTidslinje` i `KalenderLagUkeV2`** (time-akse 07–21, nå-linje, overlapp side om side — reell pikselport, ikke bare sitering), AO-00/02/04/05/09/12 (sitering + ett dokumentert avvik: AO-06/07 detaljark ikke bygget), KA-01L/02/03 (sitering — samme flate dekker uke/måned/dag), DG-01/EC-01/GP-01/BO-02/03/ME-01–04 (sitering, samt to rettelser av utdaterte Paper/`T.*`-kommentarer). Dekning i familien 105→124/204. **Deferret, IKKE løst** (se PR for begrunnelse per fil): AG-19 Notifikasjonssenter (SMS-godkjenning er ny funksjonalitet, ingen backend), AO-13 Routing-hub (fasiten krever felter `RoutingRule`-modellen ikke har — ny datamodell), JV-01/02/03 (eval-motoren finnes, men INGEN kode produserer ekte merge-forslag — ville krevd oppdiktet data), EC-02 AS Compliance (ingen MVA/skattemelding-datakilde), FY-01 Fys stall (ingen roster-ACWR-aggregering, "kjerneøkt"-klassifisering udefinert), GP-02 (fasiten vil ha stilisert flate linje-diagram, koden har ekte Mapbox — strukturelt avvik), BO-01 (fasiten er dags-slot-rutenett, koden er flertrinns wizard — strukturelt avvik), S3-01L/02/03 (T4 dokumenterte allerede 26.08 at Spiller 360-innholdet er Paper-tokens og et «eget, betydelig arbeid» — bekreftet uendret), TU-02 (generisk turneringsside, annen IA enn fasiten). | `claude/px6-agency-rest-fasit-iopubc` |
