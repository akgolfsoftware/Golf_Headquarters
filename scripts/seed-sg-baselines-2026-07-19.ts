 ...
 * Utenfor ytterste ankere klamper vi til nærmeste bånd-verdi (ingen spekulativ
 * ekstrapolering utover fasiten).
 *
 * Idempotent (upsert på @@unique([category, distanceBucket, lie])). Kjøres
 * DataGolf-syncen senere, overskriver den disse radene med ferske verdier —
 * det er ønsket oppførsel per fasitens egen note.
 *
 *   npx tsx scripts/seed-sg-baselines-2026-07-19.ts
 */
import "./_env";
...
// Lineær interpolering mellom bånd-midtpunkter, klampet i endene.
...
async function main() {
  ...
  // Bånd-midtpunkt som anker. Åpent toppbånd ("200+") ankres ved min + 25y.
  ...
  for (let low = 0; low < 250; low += 25) {
    ...
    const mid = low + 12.5;
    const expectedStrokes = Math.round(interpolate(anchors, mid) * 100) / 100;
