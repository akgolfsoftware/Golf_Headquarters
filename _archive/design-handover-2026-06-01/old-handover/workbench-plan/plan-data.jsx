/* ============================================================
   Workbench Plan · Shared data + icon helper
   ============================================================ */

function WBPIc({ id, size = 16, stroke = 1.6 }) {
  return (
...
const WBP_TREE = {
  season: 'Sesong 2026',
  periods: [
    { id: 1, name: 'Forberedelse',  status: 'done', weeks: 'uke 06–14', focus: 'FYS+TEK' },
    { id: 2, name: 'Bygging',       status: 'done', weeks: 'uke 15–18', focus: 'SLAG' },
    { id: 3, name: 'Bygging mot turnering', status: 'now',  weeks: 'uke 19–24', focus: 'SLAG+SPILL', active: true,
      subweeks: [
        { id: 19, label: 'Uke 19', state: 'done' },
        { id: 20, label: 'Uke 20', state: 'done' },
        { id: 21, label: 'Uke 21', state: 'now' },
        { id: 22, label: 'Uke 22 · Sør.åpent', state: 'peak' },
        { id: 23, label: 'Uke 23 · Tilbake',   state: 'rec' },
        { id: 24, label: 'Uke 24',  state: 'next' },
      ],
    },
    { id: 4, name: 'Peak NM-kval',  status: 'next', weeks: 'uke 25–30', focus: 'TURN' },
    { id: 5, name: 'Restitusjon',   status: 'next', weeks: 'uke 31–34', focus: 'FYS' },
  ],
};

const WBP_PLANS = [
  { id: 'A', name: 'Plan A · Mot Sør.åpent', meta: 'Aktiv · sist endret 2 min', active: true, draft: false },
  { id: 'B', name: 'Plan B · Konservativ',   meta: 'Utkast · 3 dager siden',     active: false, draft: true },
];

const WBP_TOURNAMENTS = [
...
/* ---- Period-3 pyramid weights (ideal, this phase) ----
   These are the weights that drive the lane HEIGHTS in Variant A.
   Sum to 100. Foundation phases would be heavier on FYS/TEK; this
   period (Bygging mot turnering) is heavier on SLAG/SPILL. */
const WBP_WEIGHTS = {
  fys:   15,
  tek:   18,
  slag:  27,
...
/* ---- Weeks shown in the canvas (Variant A default zoom) ---- */
const WBP_WEEKS = [
...
/* ---- Sessions for the canvas (Variant A) ----
   week = WBP_WEEKS.id ; day = 0-6 (mon-sun) ; span = days
...
const WBP_SESSIONS = [
  ...
  // Uke 22 (PEAK — Sør.åpent torsdag-lørdag = day 3-5)
...
Object.assign(window, {
  WBPIc,
  WBP_TREE, WBP_PLANS, WBP_TOURNAMENTS,
  WBP_WEIGHTS, WBP_WEEKS, WBP_SESSIONS,
});
