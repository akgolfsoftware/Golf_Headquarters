...
```jsx
<Avatar name="Øyvind Rohjan" decorative />          // i en ListRow der navnet står ved siden av
...
- `Avatar.initials(name)` er tilgjengelig som statisk hjelper når initialene trengs uten sirkelen (`Avatar.initials("Øyvind Rohjan") // "ØR"`). Verifisert på norske navn: tre ledd tar første + siste («Markus Røinås Pedersen» → MP), mellomnavn ignoreres, æøå bevares i versal («Øystein Ødegård» → ØØ), bindestrek behandles som ett ledd («Anne-Lise Berg» → AB), ettordsnavn tar to første tegn («roinas» → RO), én bokstav gir én bokstav, tomt navn gir tom sirkel.
